...
/*! 基函数的0-n阶导数 */
cv::Mat_<qreal> BSpline::DerivativeBasis(qreal u, int span, int diffOrder)
{
    u = qBound<qreal>(0, u, 1);
    Matrix ders = Matrix(diffOrder + 1, order + 1);
    Matrix ndu = Matrix(order + 1, order + 1);
    QVector<qreal> left(order + 1, 0);
    QVector<qreal> right(order + 1, 0);
    ndu(0,0) = 1;
    for (int j = 1; j <= order; ++j) {
