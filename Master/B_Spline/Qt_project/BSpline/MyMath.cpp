...
{
    /*判断A与b的维度是否一致*/
    const int row0 = A.rows - 1;
    const int col0 = A.cols - 1;
    const int dimension = b.size() - 1;
    if (row0 != col0 || row0 != dimension) {        /* 矩阵长宽不一样 */
        return QVector<qreal>();            //返回一个空的值，在外面判断是否求解成功
    }
    QVector<qreal> x(dimension + 1, 0.0);
    /*将系数矩阵上三角化*/
    for (int row1 = 0; row1 <= dimension-1; row1++) {              //从第二行开始消元
        for (int row2 = row1+1; row2 <= dimension; row2++) {
            qreal m = A(row2, row1) / A(row1, row1);
            for (int col = row1; col <= dimension; col++) {
                A(row2, col) = A(row2, col) - m * A(row1, col);
            }
            b[row2] = b[row2] - m * b[row1];
        }
    }
    x[dimension] = b[dimension] / A[dimension][dimension];
