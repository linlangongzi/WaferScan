...
void SerialPort::sendData(QVector<qreal> joints)
{
    char sum = 0;
    for (int index = 0; index < 6; ++index) {
        sum += char(joints[index]);
    }
    char data[] = {0xFF,0XFE,
                 ...
                 char(sum)};
    if (my_serialport && my_serialport->isWritable()) {
        my_serialport->write(data);
    }
}
 ...
 * \param temp
 */
void SerialPort::sendMessage(QVector<qreal> joints)
{
//    qDebug() << "Message: " << joints;
    QString data = QString("55aa000014010001ffffffff00000404");
    data += QString::number(50).sprintf("%08x",50);                         //时间码
    data += QString::number(int(joints[0])).sprintf("%08x",int(joints[0]));           //1到6号电机的长度码
    data += QString::number(int(joints[1])).sprintf("%08x",int(joints[1]));
    data += QString::number(int(joints[2])).sprintf("%08x",int(joints[2]));
    data += QString::number(int(joints[3])).sprintf("%08x",int(joints[3]));
    data += QString::number(int(joints[4])).sprintf("%08x",int(joints[4]));
    data += QString::number(int(joints[5])).sprintf("%08x",int(joints[5]));
    data += QString("12345678abcd"); //固定格式的尾码
    QByteArray ba = hexStringtoByteArray(data);
    commu->writeDatagram(ba, ba.length(), QHostAddress::Broadcast, 7408);       //7402
}
