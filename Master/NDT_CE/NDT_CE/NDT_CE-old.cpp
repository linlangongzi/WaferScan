...
//wangbingfu add end at 2010-7-29
                     ...
                     int       nCmdShow)
{
	...
	//wangbingfu add start at 2011-07-07
	//MapRegisters
	/*  rum by yachang.huang 2013-10-21
	g_pCLKPWRreg = (volatile S3C2440A_CLKPWR_REG*)VirtualAlloc(0, sizeof(S3C2440A_CLKPWR_REG), MEM_RESERVE, PAGE_NOACCESS);
	VirtualCopy((PVOID)g_pCLKPWRreg, (PVOID)(S3C2440A_BASE_REG_PA_CLOCK_POWER >> 8), sizeof(S3C2440A_CLKPWR_REG), PAGE_PHYSICAL | PAGE_READWRITE | PAGE_NOCACHE);

	
	g_pIOPregs = (volatile S3C2440A_IOPORT_REG*)VirtualAlloc(0, sizeof(S3C2440A_IOPORT_REG), MEM_RESERVE, PAGE_NOACCESS);
	VirtualCopy((PVOID)g_pIOPregs, (PVOID)(S3C2440A_BASE_REG_PA_IOPORT >> 8), sizeof(S3C2440A_IOPORT_REG), PAGE_PHYSICAL | PAGE_READWRITE | PAGE_NOCACHE);
	
	g_pIOPregs->GPHCON = (g_pIOPregs->GPHCON) & 0xffcfffff;		//关闭CLK1
//	g_pIOPregs->GPHCON = (g_pIOPregs->GPHCON) & 0xfff3ffff;		//关闭CLK0


	
	//DCLK0
	g_pIOPregs->MISCCR = (g_pIOPregs->MISCCR | 0x50) & 0xffffffdf;	//6:4 = 101
	// 5MHZ DCLK0 = PCLK / 5
	//g_pIOPregs->DCLKCON = (g_pIOPregs->DCLKCON | 0x41) & 0xffffff4d;// 7:4 = 0100 1:0 = 01
	// 25MHZ DCLK0 = PCLK/2 	PCLK = (HCLK = FCLK/4)/2 = 50(HDIVN = 2,PDIVN = 1)
	g_pIOPregs->DCLKCON = (g_pIOPregs->DCLKCON | 0x11) & 0xffffff1d;// 7:4 = 0001 1:0 = 01

	//wangbingfu add start at 2011-07-07

	//设置IP
	SetIpAddress(_T("RT2501USB1"), _T("192.168.0.83"),_T("255.255.255.0"), _T("192.168.0.1"));	//无线网卡IP
	SetIpAddress(_T("MOSUSBCE1"), _T("192.168.0.83"),_T("255.255.255.0"), _T("192.168.0.1"));	//有线网卡IP
//	SetIpAddress(_T("RT2501USB1"), _T("192.168.1.91"),_T("255.255.255.0"), _T("192.168.1.1"));
//	SetIpAddress(_T("MOSUSBCE1"), _T("192.168.1.91"),_T("255.255.255.0"), _T("192.168.1.1"));
   */
	MSG msg;
	HACCEL hAccelTable;
	...
	// wangbingfu add start on 2010-04-19
...
{
	// wangbingfu modify start on 2010-04-19
	// HWND hWnd;	
	// wangbingfu modify end on 2010-04-19
...
{
	return 0;
//  //   volatile S3C2440A_ADC_REG *ADCregs;
//     int result;
//     PHYSICAL_ADDRESS ADCCONADDR={S3C2440A_BASE_REG_PA_ADC,0};
//     ADCregs = (volatile S3C2440A_ADC_REG *)MmMapIoSpace(ADCCONADDR,sizeof(S3C2440A_ADC_REG),FALSE);
//     ADCregs->ADCCON = (1<<14)|(49<<6)|(0<<3);	//setup channel 0  1Mhz
//     ADCregs->ADCCON |=0x1;   //start ADC
// 
//     while((ADCregs->ADCCON) & 0x1);	//check if Enable_start is low
//     while(!((ADCregs->ADCCON) & 0x8000));	//check if EC(End of Conversion) flag is high
//     result=(int)(ADCregs->ADCDAT0 & 0x3ff);
//     MmUnmapIoSpace((PVOID)ADCregs, sizeof(S3C2440A_ADC_REG));
//     return (result);
}
...
LRESULT DoCreateMain(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	//	SetTimer(hWnd, TIMER_PRESS, 60000, TimerPressProc);
...
/*
	BOOL bSelfTestValue = ArmSelfTest();
	if (bSelfTestValue)
	{
	   MessageBox(NULL,_T( "自检成功"),NULL,0); 
	}
	else
	{
	   MessageBox(NULL,_T( "自检失败"),NULL,0); 
	}


    WORD wVersion = 0;
	memcpy(&wVersion,Sys_Hardware_Ver,2);
	wchar_t szTest[50];
	memset(szTest, 0, sizeof(szTest));
	swprintf(szTest, _T(" %d "), wVersion);
	MessageBox(NULL, szTest, _T("info"), MB_OK);
*/   
	...
	// Song Chenguang rem 2013-10-25
/*
	// 打开默认文件...
	// 读注册表查询上次工艺参数号
	HKEY HKeyNum;
	DWORD dwRet;
	LONG ret = RegCreateKeyEx(HKEY_CURRENT_USER, _T("ParNum"), 0, _T("REG_DWORD"), 0, 0, NULL, &HKeyNum, &dwRet);

	if(ret != ERROR_SUCCESS)
	{
		MessageBox (hWnd, TEXT ("读注册表失败"), TEXT("NDT_CE"), 0) ;
	...
	{
		// 设值
		swprintf(lpDefaultParamFile, _T("\\ResidentFlash\\1.par"));

		currentUsedParamNo = 1;
	...
	else if(dwRet == REG_OPENED_EXISTING_KEY)
	{
		...
		DWORD type = 0;
		RegQueryValueEx(HKeyNum, _T("ParNum"), 0, &type, (LPBYTE)&currentUsedParamNo, &nSize);
...
LRESULT DoPaintMain(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	if(hDrawThread == NULL)
	{
		...
		DrawThread_param.hWnd = hWnd;
		HDC hDC = GetDC(hWnd);
		DrawThread_param.hDC = hDC;

		hDrawThread = ::CreateThread(NULL, 0, DrawThreadProc, (LPVOID)&DrawThread_param, CREATE_SUSPENDED, &dwDrawThreadID);
		// 创建Event
		hDrawEvent = CreateEvent(NULL, TRUE, FALSE, TEXT("DrawEvent0"));
		
		
		ResumeThread(hDrawThread);

		SendMessage( hStatusBar,      // handle to destination control   
				 (UINT) SB_SETBKCOLOR,    // message ID   
				 0,         // = 0; not used, must be zero  
				 (LPARAM)(COLORREF) RGB(255,255,255)          // = (LPARAM) (COLORREF) clrBk;
				);
...
int AECThreadIdle(AECTHREAD_PARAM * pThreadParam)
{
...
//	MessageBox(NULL, _T("part 3"), NULL, MB_OK);
...
//	MessageBox(NULL, _T("part 4"), NULL, MB_OK);
	...
	//	WORD *ch_monitor_flag[2] = {cscan_ch1_monitor_flag, cscan_ch2_monitor_flag};
	//	gnSendCount = *cscan_ch1_monitor_flag;
//		MessageBox(NULL, _T("dwWaitObjectIndex"), NULL, MB_OK);
		for(int i=0; i<2; i++)
		{	
			//
//			if(ch_data_regs[i]->wMonitorFlag == 1)
			...
			/*	if(i == 0)
				{
					cDataFlag = cDataFlag | 2;	//|10
				}
				else if(i == 1)
				{
					cDataFlag = cDataFlag | 8; // |1000
				}
				else
				{
					;//
...
//				MessageBox(NULL, _T("before memcpy"), NULL, MB_OK);
				memcpy(&gstMonitorData[i].wPicData, &gPicData[i].data, sizeof(WORD)*DATA_SIZE);
//				MessageBox(NULL, _T("after memcpy"), NULL, MB_OK);
//				ReadCodeAndFeature(&gstMonitorData[i], i);

				if ( gbChannelSend[0] )
				{
//					MessageBox(NULL, _T("gbChannelSend"), NULL, MB_OK);
				//	int nCurrentTime = GetTickCount();					
				//	if(nCurrentTime - g_beginTime >= 25)
					{
						if(i == CurrentChannelNo)
						{
//							MessageBox(NULL, _T("i == CurrentChannelNo"), NULL, MB_OK);
							int nSendBytes = 0;
						//	char szSendData[MAX_BUFFER_SIZE];
						//	memset( szSendData, 0, sizeof(szSendData) );
							...
							stPacketHead.wChannelIndex = CurrentChannelNo;
							...
							// Song chenguang add to test
// 							if(i== 0)
// //								memset(gstMonitorData[i].wPicData, 128, sizeof(WORD)*DATA_SIZE);
// 							{
// 								int k = 0;
// 								for (int j=0; j<DATA_SIZE; j++)
// 								{
// 									k += 3;
// 									gstMonitorData[i].wPicData[j] = k;
// 
// 									if (gstMonitorData[i].wPicData[j] > 1023)
// 										gstMonitorData[i].wPicData[j] = 0;
// 								}
// 							}
// 							else
// //								memset(gstMonitorData[i].wPicData, 256, sizeof(WORD)*DATA_SIZE);
// 							{
// 								int k = 0;
// 								for (int j=0; j<DATA_SIZE; j++)
// 								{
...
// 									gstMonitorData[i].wPicData[j] = k;
// 
// 									if (gstMonitorData[i].wPicData[j] > 512)
// 										gstMonitorData[i].wPicData[j] = 0;
// 								}
// 							}

							memcpy(&szSendData[NETHEAD_SIZE], &gstMonitorData[i], stPacketHead.dwSize);
						//	SendTo(gSockClient, szSendData, NETHEAD_SIZE + stPacketHead.dwSize);	// 确保发送完
							if(send( gSockClient, szSendData, NETHEAD_SIZE + stPacketHead.dwSize, 0 )== SOCKET_ERROR)
							{
						...
						/*	if((nSendBytes = send( gSockClient, szSendData + gnTotalSendBytes, NETHEAD_SIZE + stPacketHead.dwSize - gnTotalSendBytes, 0 )) == SOCKET_ERROR)
							{
								if(GetLastError() == WSAEWOULDBLOCK)
								{
									PostMessage(hStatusBar, SB_SETTEXT, 
							...
							if(gnTotalSendBytes == NETHEAD_SIZE + stPacketHead.dwSize)
							{
								gnTotalSendBytes = 0;
							}*/
							// Song test
//							MessageBox(NULL, _T("after send"), NULL, MB_OK);
						}
				//		g_beginTime = nCurrentTime;
					}
				}
				#if 0 //测试编码值
				if(gnSendCount1 == 20)
				{
					...
					DWORD dwXHigh = cscan_code_reg->wXHigh16;
					dX = (cscan_code_reg->wXLow16 + (dwXHigh << 16)) / CODER_PER_RING_XY;
					DWORD dwYHigh = cscan_code_reg->wYHigh16;
					dY = (cscan_code_reg->wYLow16 + (dwYHigh << 16)) / CODER_PER_RING_XY;
					DWORD dwZHigh = cscan_code_reg->wZHigh16;
					dZ = (cscan_code_reg->wZLow16 + (dwZHigh << 16)) / CODER_PER_RING_Z;
				...
				*/
				/*	DWORD dwXHigh = gstMonitorData[0].stFeatureData.wXHigh16;
					dX = (gstMonitorData[0].stFeatureData.wXLow16 + (dwXHigh << 16)) / CODER_PER_RING_XY;
					DWORD dwYHigh = gstMonitorData[0].stFeatureData.wYHigh16;
					dY = (gstMonitorData[0].stFeatureData.wYLow16 + (dwYHigh << 16)) / CODER_PER_RING_XY;
					DWORD dwZHigh = gstMonitorData[0].stFeatureData.wZHigh16;
					dZ = (gstMonitorData[0].stFeatureData.wZLow16 + (dwZHigh << 16)) / CODER_PER_RING_Z;
				*/	
				/*	wchar_t strText[20];
					memset(strText, 0, sizeof(strText));			
					swprintf(strText, _T("%.1f"), dX); 
					DrawLableText(hBtnMode1or2, strText, DEFAULT_LABLE_TEXT_HEIGHT, STATIC_TEXT_COLOR, LABEL_START_X, LABEL_5_START_Y);
					memset(strText, 0, sizeof(strText));			
					swprintf(strText, _T("%.1f"), dY);
					DrawLableText(hBtnMode1plus2, strText, DEFAULT_LABLE_TEXT_HEIGHT, STATIC_TEXT_COLOR, LABEL_START_X, LABEL_5_START_Y);
					memset(strText, 0, sizeof(strText));			
					swprintf(strText, _T("%.1f"), dZ); 
					DrawLableText(hBtnMode1to2, strText, DEFAULT_LABLE_TEXT_HEIGHT, STATIC_TEXT_COLOR, LABEL_START_X, LABEL_5_START_Y);

					memset(strText, 0, sizeof(strText));			
					swprintf(strText, _T("%d"), cscan_code_reg->wXHigh16); 
					DrawLableText(hBtnParamSend, strText, DEFAULT_LABLE_TEXT_HEIGHT, STATIC_TEXT_COLOR, LABEL_START_X, LABEL_5_START_Y);
					memset(strText, 0, sizeof(strText));			
					swprintf(strText, _T("%d"), cscan_code_reg->wXLow16);
					DrawLableText(hBtnParamRecv, strText, DEFAULT_LABLE_TEXT_HEIGHT, STATIC_TEXT_COLOR, LABEL_START_X, LABEL_5_START_Y);
					memset(strText, 0, sizeof(strText));			
					swprintf(strText, _T("%d"), cscan_code_reg->wZHigh16); 
					DrawLableText(hBtnParamDisplay, strText, DEFAULT_LABLE_TEXT_HEIGHT, STATIC_TEXT_COLOR, LABEL_START_X, LABEL_5_START_Y);					
					memset(strText, 0, sizeof(strText));			
					swprintf(strText, _T("%d"), cscan_code_reg->wZLow16); 
					DrawLableText(hBtnParamCurve, strText, DEFAULT_LABLE_TEXT_HEIGHT, STATIC_TEXT_COLOR, LABEL_START_X, LABEL_5_START_Y);
				//	memset(strText, 0, sizeof(strText));			
				//	swprintf(strText, _T("%d"), cscan_code_reg->); 
				//	DrawLableText(hBtnParamGate, strText, DEFAULT_LABLE_TEXT_HEIGHT, STATIC_TEXT_COLOR, LABEL_START_X, LABEL_5_START_Y);
				*/
				}
				#endif
				...
				{
					gnSendCount1 = 0;
				 	float fSyncRange1 = Dot2Range(6000, gstMonitorData[0].stFeatureData.wGate1SyncTime);
					float fMPRange1 = Dot2Range(6000, gstMonitorData[0].stFeatureData.wGate1MPTime);
					WORD wMPAmp1 = (WORD)(gstMonitorData[0].stFeatureData.wGate1MPAmp / MAX_HEIGHT * 100.0f);
					float fFPRange1 = Dot2Range(6000, gstMonitorData[0].stFeatureData.wGate1FPTime);
					WORD wFPAmp1 = (WORD)(gstMonitorData[0].stFeatureData.wGate1FPAmp / MAX_HEIGHT * 100.0f);

					float fSyncRange2 = Dot2Range(6000, gstMonitorData[0].stFeatureData.wGate2SyncTime);
					float fMPRange2 = Dot2Range(6000, gstMonitorData[0].stFeatureData.wGate2MPTime);
					WORD wMPAmp2 = (WORD)(gstMonitorData[0].stFeatureData.wGate2MPAmp / MAX_HEIGHT * 100.0f);
					float fFPRange2 = Dot2Range(6000, gstMonitorData[0].stFeatureData.wGate2FPTime);
					WORD wFPAmp2 = (WORD)(gstMonitorData[0].stFeatureData.wGate2FPAmp / MAX_HEIGHT * 100.0f);
				
				/*	float fSyncRange1 = Dot2Range(6000, cscan_ch1_feature_reg->wGate1SyncTime);
					float fMPRange1 = Dot2Range(6000, cscan_ch1_feature_reg->wGate1MPTime);
					WORD wMPAmp1 = (WORD)(cscan_ch1_feature_reg->wGate1MPAmp / MAX_HEIGHT * 100.0f);
					float fFPRange1 = Dot2Range(6000, cscan_ch1_feature_reg->wGate1FPTime);
					WORD wFPAmp1 = (WORD)(cscan_ch1_feature_reg->wGate1FPAmp / MAX_HEIGHT * 100.0f);

					float fSyncRange2 = Dot2Range(6000, cscan_ch1_feature_reg->wGate2SyncTime);
					float fMPRange2 = Dot2Range(6000, cscan_ch1_feature_reg->wGate2MPTime);
					WORD wMPAmp2 = (WORD)(cscan_ch1_feature_reg->wGate2MPAmp / MAX_HEIGHT * 100.0f);
					float fFPRange2 = Dot2Range(6000, cscan_ch1_feature_reg->wGate2FPTime);
					WORD wFPAmp2 = (WORD)(cscan_ch1_feature_reg->wGate2FPAmp / MAX_HEIGHT * 100.0f);
				*/	
					wchar_t strText[20];
					memset(strText, 0, sizeof(strText));			
					swprintf(strText, _T("%.1f"), fSyncRange1); 
					DrawLableText(hBtnMode1or2, strText, DEFAULT_LABLE_TEXT_HEIGHT, STATIC_TEXT_COLOR, LABEL_START_X, LABEL_5_START_Y);
					memset(strText, 0, sizeof(strText));			
					swprintf(strText, _T("%.1f"), fMPRange1); //cscan_ch1_feature_reg->wGate1MPTime
					DrawLableText(hBtnMode1plus2, strText, DEFAULT_LABLE_TEXT_HEIGHT, STATIC_TEXT_COLOR, LABEL_START_X, LABEL_5_START_Y);
					memset(strText, 0, sizeof(strText));			
					swprintf(strText, _T("%d"), wMPAmp1); 
					DrawLableText(hBtnMode1to2, strText, DEFAULT_LABLE_TEXT_HEIGHT, STATIC_TEXT_COLOR, LABEL_START_X, LABEL_5_START_Y);					
					memset(strText, 0, sizeof(strText));			
					swprintf(strText, _T("%.1f"), fFPRange1); 
					DrawLableText(hBtnModeDanjing, strText, DEFAULT_LABLE_TEXT_HEIGHT, STATIC_TEXT_COLOR, LABEL_START_X, LABEL_5_START_Y);
					memset(strText, 0, sizeof(strText));			
					swprintf(strText, _T("%d"), wFPAmp1); 
					DrawLableText(hBtnModeShuangjing, strText, DEFAULT_LABLE_TEXT_HEIGHT, STATIC_TEXT_COLOR, LABEL_START_X, LABEL_5_START_Y);
					
					memset(strText, 0, sizeof(strText));			
					swprintf(strText, _T("%.1f"), fSyncRange2); 
					DrawLableText(hBtnParamSend, strText, DEFAULT_LABLE_TEXT_HEIGHT, STATIC_TEXT_COLOR, LABEL_START_X, LABEL_5_START_Y);
					memset(strText, 0, sizeof(strText));			
					swprintf(strText, _T("%.1f"), fMPRange2); //cscan_ch1_feature_reg->wGate1MPTime
					DrawLableText(hBtnParamRecv, strText, DEFAULT_LABLE_TEXT_HEIGHT, STATIC_TEXT_COLOR, LABEL_START_X, LABEL_5_START_Y);
					memset(strText, 0, sizeof(strText));			
					swprintf(strText, _T("%d"), wMPAmp2); 
					DrawLableText(hBtnParamDisplay, strText, DEFAULT_LABLE_TEXT_HEIGHT, STATIC_TEXT_COLOR, LABEL_START_X, LABEL_5_START_Y);					
					memset(strText, 0, sizeof(strText));			
					swprintf(strText, _T("%.1f"), fFPRange2); 
					DrawLableText(hBtnParamCurve, strText, DEFAULT_LABLE_TEXT_HEIGHT, STATIC_TEXT_COLOR, LABEL_START_X, LABEL_5_START_Y);
					memset(strText, 0, sizeof(strText));			
					swprintf(strText, _T("%d"), wFPAmp2); 
					DrawLableText(hBtnParamGate, strText, DEFAULT_LABLE_TEXT_HEIGHT, STATIC_TEXT_COLOR, LABEL_START_X, LABEL_5_START_Y);

					memset(strText, 0, sizeof(strText));			
					swprintf(strText, _T("%d"), cDataFlag);
					DrawLableText(hBtnOtherCfgStoreMode, strText, DEFAULT_LABLE_TEXT_HEIGHT, STATIC_TEXT_COLOR, LABEL_START_X, LABEL_5_START_Y);
				}
				#endif
			}
		}
...
//  Song rem
		...
		memset(strText, 0, sizeof(strText));			
	//	swprintf(strText, _T("%d"), cDataFlag);
		swprintf(strText, _T("%d"), gnSendCount1);
		DrawLableText(hBtnOtherCfgStoreMode, strText, DEFAULT_LABLE_TEXT_HEIGHT, STATIC_TEXT_COLOR, LABEL_START_X, LABEL_5_START_Y);
...
*/
	/*	for(WORD i=0; i<2; i++)
		{			
			if(ch_data_regs[i]->wMonitorFlag == 1)
			{
				if((i==0 && mode == SingleChannelB) || 
					(i==1 && mode == SingleChannelA) ||
					(i==0 && mode == OneSendOneReceive))
				{
					gPicData[i].bDrawed = FALSE;
					continue;
				}
				...
				// 读取波形数据
				if(bAdjustStart  || bAngleAdjustStart || bChWaveSpeedAdjustStart || bChZeroSingleStart || bChZeroDoubleMinStart || bChZeroDoubleStart)
				{
#if 1//_NDT_TEST
					// 读取通道1闸门1的伤时，伤幅
					if( i == 0 && nPeakCount < 100)
					{
						PeakInfoTest[nPeakCount].wDefectTime = ch1_gate_reg->wGate1DefectTime;
						PeakInfoTest[nPeakCount].wDefectAmp = ch1_gate_reg->wGate1DefectAmp;

						PeakInfoTest2[nPeakCount].wDefectTime = ch1_gate_reg->wGate2DefectTime;
						PeakInfoTest2[nPeakCount].wDefectAmp = ch1_gate_reg->wGate2DefectAmp;
					}
#endif
					PutsReadDrawData(gPicData[i], ch_data_regs[i], ch_datas[i], i, PeakInfoSum[i]);		
				
					// songchenguang add start 2011-03-02
					nPeakCount++;

					break;
					// songchenguang add end 2011-03-02
				}
				else
					PutsReadDrawData(gPicData[i], ch_data_regs[i], ch_datas[i], i, NULL);
		...
		if ( gbChannelSend[0] )
		{
			...
			//	memset( szSendData, 0, sizeof(szSendData) );
				...
				stPacketHead.wGateIndex = 0;
				stPacketHead.dwSize = sizeof(CSCAN_CH_SEND_MONITOR_DATA_t)*2;

				memcpy(szSendData, &stPacketHead, sizeof(stPacketHead));
				memcpy(&szSendData[NETHEAD_SIZE], &gstMonitorData, stPacketHead.dwSize);
				if(send( gSockClient, szSendData, NETHEAD_SIZE + stPacketHead.dwSize, 0 )== SOCKET_ERROR)
				{
					if(GetLastError() == WSAEWOULDBLOCK)
					{
		...
		//	memset( szSendData, 0, sizeof(szSendData) );
			...
			for(int i = 0; i < 2; i++)
			{	
//				if(ch_data_regs[i]->wFeatureReadFlag == 1) // Song rem
				{
				/*	if(i == 0)
					{
						cDataFlag = cDataFlag | 1;
					}
					else if(i == 1)
					{
						cDataFlag = cDataFlag | 4;
					}
					else
					{
						;//
					...
					{
						gnSendCount = 1;
				/*		stFeatureData[i][0].wXLow16 = 100 + gnSendCount;
						stFeatureData[i][0].wXHigh16 = 100 + gnSendCount;
						stFeatureData[i][0].wYLow16 = 100 + gnSendCount;
						stFeatureData[i][0].wYHigh16 = 100 + gnSendCount;
						stFeatureData[i][0].wZLow16 = 100 + gnSendCount;
						stFeatureData[i][0].wZHigh16 = 100 + gnSendCount;*/

						stPacketHead.wChannelIndex = i;
						memcpy(szSendData, &stPacketHead, sizeof(stPacketHead));						
						memcpy(&szSendData[NETHEAD_SIZE], ch_feature_buffer[i][0], stPacketHead.dwSize);

					//	SendTo( gSockClient2, szSendData, NETHEAD_SIZE + stPacketHead.dwSize );
...
*/
					...
					{
						gnSendCount = 2;
				/*		stFeatureData[i][0].wXLow16 = 200 + gnSendCount;
						stFeatureData[i][0].wXHigh16 = 200 + gnSendCount;
						stFeatureData[i][0].wYLow16 = 200 + gnSendCount;
						stFeatureData[i][0].wYHigh16 = 200 + gnSendCount;
						stFeatureData[i][0].wZLow16 = 200 + gnSendCount;
						stFeatureData[i][0].wZHigh16 = 200 + gnSendCount;*/
...
*/
					...
					if(bFeatureFlag[i])
					{
						...
						memcpy(szSendData, &stPacketHead, sizeof(stPacketHead));						
						memcpy(&szSendData[NETHEAD_SIZE], &stFeatureData[i], stPacketHead.dwSize);
						
					//	send( gSockClient2, szSendData, NETHEAD_SIZE + stPacketHead.dwSize, 0 );
					/*	if(gnTotalSendBytes < NETHEAD_SIZE + stPacketHead.dwSize)
						{
							if((nSendBytes = send( gSockClient2, szSendData + gnTotalSendBytes, NETHEAD_SIZE + stPacketHead.dwSize - gnTotalSendBytes, 0 )) == SOCKET_ERROR)
							{
								if(GetLastError() == WSAEWOULDBLOCK)
								{
									PostMessage(hStatusBar, SB_SETTEXT, 
		...
		*/
		...
		{	
			#if 0
		/*	REAL_SEND_DATA_t stRealSendData;
			memset( &stRealSendData, 0, sizeof(stRealSendData) );
			...
			stPacketHead.wChannelIndex = 0;
			stPacketHead.wGateIndex = 0;
			stPacketHead.dwSize =  sizeof(stRealSendData);

			for(int i = 0; i < CHANNEL_NUM; i++)
			{
				memcpy(stRealSendData.wPicData[i], gPicData[i].data, sizeof(gPicData[i].data));
				for(int j = 0; j < GATE_NUM - 1; j++)
				{
					stRealSendData.stPeakInfo[i][j] = gstConvertPeakInfo[i][j];
				//	stRealSendData.stPeakInfo[i][j] = gPeakDefectInfos[i][j];
					
				}
				
			}
			memcpy(szSendData, &stPacketHead, sizeof(stPacketHead));
			memcpy(&szSendData[NETHEAD_SIZE], &stRealSendData, sizeof(stRealSendData));

			send( gSockClient, szSendData, NETHEAD_SIZE + sizeof(stRealSendData), 0 );
		*/
			char szSendData[16050];
			memset( szSendData, 0, sizeof(szSendData) );
			send( gSockClient, szSendData, NETHEAD_SIZE + 16000, 0 );
			#else
		/*	if(cscan_ch1_data_reg->wFeatureReadFlag == 1)
			{
				cDataFlag = cDataFlag | 0x1;
				if(*cscan_ch1_feature_buffer_flag == 1)	//缓冲区1可读
				{
					for(int i = 0; i < 128; i++)
					{
						memcpy(&stFeatureData[0][i], cscan_ch1_feature_buffer1 + i*32, nPerFeatureSize);
					}
				}
				else if(*cscan_ch1_feature_buffer_flag == 2)	//缓冲区2可读
				{
					for(int i = 0; i < 128; i++)
					{
						memcpy(&stFeatureData[0][i], cscan_ch1_feature_buffer2 + i*32, nPerFeatureSize);
					}
				}
				else
				{
				...
				cscan_ch1_data_reg->wFeatureReadReset = 1;
				Sleep(1);	
				cscan_ch1_data_reg->wFeatureReadReset = 0;
			}

			if(cscan_ch1_data_reg->wMonitorFlag == 1)
			{
				cDataFlag = cDataFlag | 0x10;
				memcpy(&gstMonitorData[0].wPicData, cscan_ch1_data, sizeof(WORD)*DATA_SIZE);
				cscan_ch1_data_reg->wMonitorReset = 1;
				Sleep(1);	
				cscan_ch1_data_reg->wMonitorReset = 0;
				ReadCodeAndFeature(&gstMonitorData[i], 0);
			}

			if(cscan_ch2_data_reg->wFeatureReadFlag == 1)
			{
				cDataFlag = cDataFlag | 0x100;
				if(*cscan_ch1_feature_buffer_flag == 1)	//缓冲区1可读
				{
					for(int i = 0; i < 128; i++)
					{
						memcpy(&stFeatureData[1][i], cscan_ch2_feature_buffer1 + i*32, nPerFeatureSize);
					}
				}
				else if(*cscan_ch1_feature_buffer_flag == 2)	//缓冲区2可读
				{
					for(int i = 0; i < 128; i++)
					{
						memcpy(&stFeatureData[1][i], cscan_ch2_feature_buffer2 + i*32, nPerFeatureSize);
					}
				}
				else
				{
			...
			if(cscan_ch2_data_reg->wMonitorFlag == 1)
			{
				cDataFlag = cDataFlag | 0x1000;
				memcpy(&gstMonitorData[1].wPicData, cscan_ch2_data, sizeof(WORD)*DATA_SIZE);
				cscan_ch2_data_reg->wMonitorReset = 1;
				Sleep(1);	
				cscan_ch2_data_reg->wMonitorReset = 0;
				ReadCodeAndFeature(&gstMonitorData[i], 1);
			}
			*/
			for(int i=0; i<2; i++)
			{	
				if(ch_data_regs[i]->wFeatureReadFlag == 1)
				{
					...
					if(*ch_feature_buffer_flag[i] == 1)	//缓冲区1可读
					{
						for(int j = 0; j < 128; j++)
						{
							memcpy(&stFeatureData[i][j], ch_feature_buffer[i] + j*32, nPerFeatureSize);
						}
					}
					else if(*ch_feature_buffer_flag[i] == 2)	//缓冲区2可读
					{
			...
			int nFeatureSize = sizeof(CH_FEATURE_BUFFER_DATA_t)*128;
			int nMonitorSize = sizeof(CSCAN_CH_SEND_MONITOR_DATA_t);
			if(cDataFlag == 1)	//通道1有特征数据 1
			{
				stPacketHead.dwSize = nFeatureSize + 1;
				memcpy(szSendData, &stPacketHead, sizeof(stPacketHead));
				memcpy(&szSendData[NETHEAD_SIZE], &cDataFlag, 1);
				memcpy(&szSendData[NETHEAD_SIZE + 1], &stFeatureData[0], nFeatureSize);
		//		send( gSockClient, szSendData, NETHEAD_SIZE + stPacketHead.dwSize, 0 );	
		//		gnSendCount++;
			}
			else if(cDataFlag == 2)		//通道1有监视数据 10
			{
				stPacketHead.dwSize = sizeof(CSCAN_CH_SEND_MONITOR_DATA_t) + 1;
				memcpy(szSendData, &stPacketHead, sizeof(stPacketHead));
				memcpy(&szSendData[NETHEAD_SIZE], &cDataFlag, 1);
				memcpy(&szSendData[NETHEAD_SIZE + 1], &gstMonitorData[0], sizeof(CSCAN_CH_SEND_MONITOR_DATA_t));
				send( gSockClient, szSendData, NETHEAD_SIZE + stPacketHead.dwSize, 0 );	
				gnSendCount++;
			}
			else if(cDataFlag == 4)		//通道2有特征数据 100
			{
				stPacketHead.dwSize = nFeatureSize + 1;
				memcpy(szSendData, &stPacketHead, sizeof(stPacketHead));
				memcpy(&szSendData[NETHEAD_SIZE], &cDataFlag, 1);
				memcpy(&szSendData[NETHEAD_SIZE + 1], &stFeatureData[1], nFeatureSize);
		//		send( gSockClient, szSendData, NETHEAD_SIZE + stPacketHead.dwSize, 0 );	
		//		gnSendCount++;
			}
			else if(cDataFlag == 8)	//通道2有监视数据 1000
			{
				stPacketHead.dwSize = sizeof(CSCAN_CH_SEND_MONITOR_DATA_t) + 1;
				memcpy(szSendData, &stPacketHead, sizeof(stPacketHead));
				memcpy(&szSendData[NETHEAD_SIZE], &cDataFlag, 1);
				memcpy(&szSendData[NETHEAD_SIZE + 1], &gstMonitorData[1], sizeof(CSCAN_CH_SEND_MONITOR_DATA_t));
		//		send( gSockClient, szSendData, NETHEAD_SIZE + stPacketHead.dwSize, 0 );	
		//		gnSendCount++;
			}
			else if(cDataFlag == 3)		//通道1有特征数据,通道1有监视数据 11
			{			
				stPacketHead.dwSize = nFeatureSize + nMonitorSize + 1;
				memcpy(szSendData, &stPacketHead, sizeof(stPacketHead));
				memcpy(&szSendData[NETHEAD_SIZE], &cDataFlag, 1);
				memcpy(&szSendData[NETHEAD_SIZE + 1], &stFeatureData[0], nFeatureSize);
				memcpy(&szSendData[NETHEAD_SIZE + 1 + nFeatureSize], &gstMonitorData[0], nMonitorSize);
		//		send( gSockClient, szSendData, NETHEAD_SIZE + stPacketHead.dwSize, 0 );	
		//		gnSendCount++;
			}
			else if(cDataFlag == 5)		//通道1有特征数据,通道2有特征数据 101
			{
				stPacketHead.dwSize = nFeatureSize * 2 + 1;
				memcpy(szSendData, &stPacketHead, sizeof(stPacketHead));
				memcpy(&szSendData[NETHEAD_SIZE], &cDataFlag, 1);
				memcpy(&szSendData[NETHEAD_SIZE + 1], &stFeatureData[0], nFeatureSize);
				memcpy(&szSendData[NETHEAD_SIZE + 1 + nFeatureSize], &stFeatureData[1], nFeatureSize);
		//		send( gSockClient, szSendData, NETHEAD_SIZE + stPacketHead.dwSize, 0 );	
		//		gnSendCount++;
			}
			else if(cDataFlag == 6)		//通道1有监视数据,通道2有特征数据 110
			{
				stPacketHead.dwSize = nFeatureSize + nMonitorSize + 1;
				memcpy(szSendData, &stPacketHead, sizeof(stPacketHead));
				memcpy(&szSendData[NETHEAD_SIZE], &cDataFlag, 1);
				memcpy(&szSendData[NETHEAD_SIZE + 1], &gstMonitorData[0], nMonitorSize);
				memcpy(&szSendData[NETHEAD_SIZE + 1 + nMonitorSize], &stFeatureData[1], nFeatureSize);
		//		send( gSockClient, szSendData, NETHEAD_SIZE + stPacketHead.dwSize, 0 );	
		//		gnSendCount++;
			}
			else if(cDataFlag == 7)		//通道1有特征数据,通道1有监视数据,通道2有特征数据 111
			{
				stPacketHead.dwSize = nFeatureSize*2 + nMonitorSize + 1;
				memcpy(szSendData, &stPacketHead, sizeof(stPacketHead));
				memcpy(&szSendData[NETHEAD_SIZE], &cDataFlag, 1);
				memcpy(&szSendData[NETHEAD_SIZE + 1], &stFeatureData[0], nFeatureSize);
				memcpy(&szSendData[NETHEAD_SIZE + 1 + nFeatureSize], &gstMonitorData[0], nMonitorSize);
				memcpy(&szSendData[NETHEAD_SIZE + 1 + nFeatureSize + nMonitorSize], &stFeatureData[1], nFeatureSize);
		//		send( gSockClient, szSendData, NETHEAD_SIZE + stPacketHead.dwSize, 0 );	
		//		gnSendCount++;
			}
			else if(cDataFlag == 9)		//通道1有特征数据,通道2有监视数据 1001
			{
				memcpy(szSendData, &stPacketHead, sizeof(stPacketHead));
				memcpy(&szSendData[NETHEAD_SIZE], &cDataFlag, 1);
				memcpy(&szSendData[NETHEAD_SIZE + 1], &stFeatureData[0], nFeatureSize);
				memcpy(&szSendData[NETHEAD_SIZE + 1 + nFeatureSize], &gstMonitorData[1], nMonitorSize);
		//		send( gSockClient, szSendData, NETHEAD_SIZE + stPacketHead.dwSize, 0 );	
		//		gnSendCount++;
			}
			else if(cDataFlag == 10)		//通道1有监视数据,通道2有监视数据 1010
			{
				stPacketHead.dwSize = nMonitorSize*2 + 1;
				memcpy(szSendData, &stPacketHead, sizeof(stPacketHead));
				memcpy(&szSendData[NETHEAD_SIZE], &cDataFlag, 1);
				memcpy(&szSendData[NETHEAD_SIZE + 1], &gstMonitorData[0], nMonitorSize);
				memcpy(&szSendData[NETHEAD_SIZE + 1 + nMonitorSize], &gstMonitorData[1], nMonitorSize);
		//		send( gSockClient, szSendData, NETHEAD_SIZE + stPacketHead.dwSize, 0 );	
		//		gnSendCount++;
			}
			else if(cDataFlag == 11)		//通道1有特征数据,通道1有监视数据,通道2有监视数据 1011
			{
				stPacketHead.dwSize = nFeatureSize + nMonitorSize*2 + 1;
				memcpy(szSendData, &stPacketHead, sizeof(stPacketHead));
				memcpy(&szSendData[NETHEAD_SIZE], &cDataFlag, 1);
				memcpy(&szSendData[NETHEAD_SIZE + 1], &stFeatureData[0], nFeatureSize);
				memcpy(&szSendData[NETHEAD_SIZE + 1 + nFeatureSize], &gstMonitorData[0], nMonitorSize);
				memcpy(&szSendData[NETHEAD_SIZE + 1 + nFeatureSize + nMonitorSize], &gstMonitorData[1], nMonitorSize);
		//		send( gSockClient, szSendData, NETHEAD_SIZE + stPacketHead.dwSize, 0 );	
		//		gnSendCount++;
			}
			else if(cDataFlag == 12)		//通道2有特征数据,通道2有监视数据 1100
			{
				stPacketHead.dwSize = nFeatureSize + nMonitorSize + 1;
				memcpy(szSendData, &stPacketHead, sizeof(stPacketHead));
				memcpy(&szSendData[NETHEAD_SIZE], &cDataFlag, 1);
		...
		//		gnSendCount++;
			...
			else if(cDataFlag == 13)		//通道1有特征数据,通道2有特征数据,通道2有监视数据 1101
			{
				stPacketHead.dwSize = nFeatureSize*2 + nMonitorSize + 1;
				memcpy(szSendData, &stPacketHead, sizeof(stPacketHead));
				memcpy(&szSendData[NETHEAD_SIZE], &cDataFlag, 1);
				memcpy(&szSendData[NETHEAD_SIZE + 1], &stFeatureData[0], nFeatureSize);
				memcpy(&szSendData[NETHEAD_SIZE + 1 + nFeatureSize], &stFeatureData[1], nFeatureSize);
				memcpy(&szSendData[NETHEAD_SIZE + 1 + nFeatureSize *2], &gstMonitorData[1], nMonitorSize);
		//		send( gSockClient, szSendData, NETHEAD_SIZE + stPacketHead.dwSize, 0 );	
		//		gnSendCount++;
			}
			else if(cDataFlag == 14)		//通道1有监视数据,通道2有特征数据,通道2有监视数据 1110
			{
				stPacketHead.dwSize = nFeatureSize + nMonitorSize*2 + 1;
				memcpy(szSendData, &stPacketHead, sizeof(stPacketHead));
				memcpy(&szSendData[NETHEAD_SIZE], &cDataFlag, 1);
				memcpy(&szSendData[NETHEAD_SIZE + 1], &gstMonitorData[0], nMonitorSize);
				memcpy(&szSendData[NETHEAD_SIZE + 1 + nMonitorSize], &stFeatureData[1], nFeatureSize);
				memcpy(&szSendData[NETHEAD_SIZE + 1 + nFeatureSize + nFeatureSize], &gstMonitorData[1], nMonitorSize);
		//		send( gSockClient, szSendData, NETHEAD_SIZE + stPacketHead.dwSize, 0 );	
		//		gnSendCount++;
			}
			else if(cDataFlag == 15)		//通道1有特征数据,通道1有监视数据,通道2有特征数据,通道2有监视数据 1111
			{
				stPacketHead.dwSize = nFeatureSize*2 + nMonitorSize*2 + 1;
				memcpy(szSendData, &stPacketHead, sizeof(stPacketHead));
				memcpy(&szSendData[NETHEAD_SIZE], &cDataFlag, 1);
				memcpy(&szSendData[NETHEAD_SIZE + 1], &stFeatureData[0], nFeatureSize);
				memcpy(&szSendData[NETHEAD_SIZE + 1 + nFeatureSize], &gstMonitorData[0], nMonitorSize);
				memcpy(&szSendData[NETHEAD_SIZE + 1 + nFeatureSize + nMonitorSize], &stFeatureData[1], nFeatureSize);
				memcpy(&szSendData[NETHEAD_SIZE + 1 + nFeatureSize*2 + nMonitorSize], &gstMonitorData[1], nMonitorSize);
		//		send( gSockClient, szSendData, NETHEAD_SIZE + stPacketHead.dwSize, 0 );	
		//		gnSendCount++;
			}
	/*		if(cDataFlag != 0)
		...
		/*	wchar_t strText[20];
			memset(strText, 0, sizeof(strText));			
			swprintf(strText, _T("%d"), gnSendCount); 
			DrawLableText(hBtnOtherCfgPrint, strText, DEFAULT_LABLE_TEXT_HEIGHT, STATIC_TEXT_COLOR, LABEL_START_X, LABEL_5_START_Y);
...
//			nPeakCount++;
			...
			{				
				...
				for(int i=0; i<2; i++)
				{
					for(int j=0; j<2; j++)
					{
						gPeakInfos[i][j].wDefectTime = PeakInfoSum[i][j].wDefectTime / nPeakCount;
						gPeakInfos[i][j].wDefectAmp = PeakInfoSum[i][j].wDefectAmp / nPeakCount;
					}
				}
				...
				DWORD dwWriteDataSize;
				hFileWrite = CreateFile(_T("\\ResidentFlash\\shangshiData.txt"),
										GENERIC_READ|GENERIC_WRITE,
										0,
										NULL,
										CREATE_ALWAYS,
				...
				for(int i=0; i<100; i++)
				{
					// 计算声速、零点
					WORD nWaveSpeed =  
							WORD( (80.0f - 40.0f)* ((ADFREQUENCY*1000000) / 1000) / ((PeakInfoTest2[i].wDefectTime - PeakInfoTest[i].wDefectTime)/2.0) );

					// 计算TS1'
					float nNewTimeS1 = 40.0f / (nWaveSpeed * 1000 ) *1000000; 
					// 计算新的修正值，发下去
					float fAmend = SendParam_ad[0].nAmend + (PeakInfoTest[i].wDefectTime / float(ADFREQUENCY) - nNewTimeS1*2);
					
					char szText[50];
					memset(szText, 0, sizeof(szText));
					sprintf(szText, "%.0f\t%.0f\t%.0f\t%.0f\t%d\t%.1f\t\r\n", PeakInfoTest[i].wDefectTime, PeakInfoTest[i].wDefectAmp,
						PeakInfoTest2[i].wDefectTime, PeakInfoTest2[i].wDefectAmp, nWaveSpeed, fAmend);
...
#endif
				nPeakCount = 0;
				memset(PeakInfoSum, 0, sizeof(PeakInfoSum));

				if(bAdjustStart)
				{
					AutoJustParam(NULL);
					PostMessage(currentAdjustBtn, WM_KEYDOWN, (WPARAM)'9', (LPARAM)0);
				}
				if(bAngleAdjustStart)
				{
					angleAdjust();
					PostMessage(currentAdjustBtn, WM_KEYDOWN, (WPARAM)'9', (LPARAM)0);
				}
				
				if(bChZeroSingleStart)
				{
					gfAdjustZero = zeroAdjust(gfZeroThickness);
					PostMessage(currentChZeroSingleBtn, WM_KEYDOWN, (WPARAM)'9', (LPARAM)0);
				}
				if(bChZeroDoubleStart)
				{
					float gfAdjustZeroMax = zeroAdjust(fMaxThickness);
					gfAdjustZero = (gfAdjustZeroMin + gfAdjustZeroMax) / 2;
					PostMessage(currentChZeroDoubleBtn, WM_KEYDOWN, (WPARAM)'9', (LPARAM)0);
				}
				if(bChZeroDoubleMinStart)
				{
					gfAdjustZeroMin = zeroAdjust(fMinThickness);
					PostMessage(currentChZeroDoubleBtn, WM_KEYDOWN, (WPARAM)'9', (LPARAM)0);
				}
				if(bChWaveSpeedAdjustStart)
				{
					gwAdjustWaveSpeed = waveSpeedAdjust(gfSpeedAdjustThickness);
					PostMessage(currentChWaveSpeedAdjustBtn, WM_KEYDOWN, (WPARAM)'9', (LPARAM)0);
				}
				InitializeCriticalSection(&csAdjust);
				EnterCriticalSection(&csAdjust);
				
				gbAdjustStart = FALSE;
				gbAngleAdjustStart = FALSE;
				gbAvgPeakFinished = TRUE;
				gbChZeroSingleStart = FALSE;
				gbChZeroDoubleStart = FALSE;
				gbChZeroDoubleMinStart = FALSE;
				gbChWaveSpeedAdjustStart = FALSE;
		...
		// songchenguang add start 2010-10-09
		// 从画图线程挪过来的，判断是否报警		
		if(!gbBscanSwitch && !gbMeasureSwitch)
		{
			alarmProcess(mode);
		}
	/*	else
		...
		}*/ //wangbingfu delete at  2011-3-28
		// songchenguang add end 2010-10-09
...
/*	if(gCurrentWnd != NULL && (gCurrentWnd == hWndTlMenu || gCurrentWnd == hWndTlAdjust) && gwDeviceType == 2)
	{
		bSaveFlag = FALSE;
	}
	else
	...
	//wangbingfu add end at 2011-3-1
	switch(wMsg)
	{
	case WM_KEYUP:
	...
	case WM_KEYDOWN:
		if(!bFreezeUp)	// 未冻结
		{
			...
			/*	CURVEPOINT_DATA tmpSelectedDgsPointData[GATE_NUM][MAX_CURVE_POINT_NUM];
				...
				//wangbingfu add start at 2010-7-29 改变DAC闸门线
				
				//wangbingfu add end at 2010-7-29
				//else if(bFreezeUp) // 右移波峰点
				//{
				//	FreezeRightMove();
				//	DrawPeakInfo();
				//}
...
DWORD WINAPI SocketThreadProc( LPVOID lpParameter )
{
    ...
    MessageBox(NULL, _T("SocketThreadProc"), NULL, MB_OK);
...
//	addr.sin_port= htons( COMMUNICATION_MONITOR_PORT ); // 保证字节顺序
	addr.sin_port= htons( 3333 ); // 保证字节顺序
//	addr.sin_port= htons( 7777 ); // 保证字节顺序
		...
		switch(stNetworkEvents.lNetworkEvents)
		{
			case FD_ACCEPT://服务器端接收到连接请求后
				{	
					wchar_t strText[20];
					memset(strText, 0, sizeof(strText));
					swprintf(strText, _T("accept1%d"), nIndex);
					PostMessage(hStatusBar, SB_SETTEXT, 
						(WPARAM) (5 | (SBT_OWNERDRAW| SBT_NOBORDERS) ), (LPARAM)(LPWSTR)strText);
				//	MessageBox(hWndMain, _T("accept"), _T("accept"), MB_OK);

					if ( stNetworkEvents.iErrorCode[FD_ACCEPT_BIT] != 0 )  // 发生网络错误
					{
						//MessageBox(hWndMain, _T("send error"), _T("send error"), MB_OK);
						PostMessage(hStatusBar, SB_SETTEXT, 
						(WPARAM) (5 | (SBT_OWNERDRAW| SBT_NOBORDERS) ), (LPARAM)(LPWSTR)_T("acceptError"));
						break;
					}
					SOCKET sockAccept;
			...
			/*		wchar_t strText[20];	
					memset(strText, 0, sizeof(strText));
					swprintf(strText, _T("%.d"), nEventTotal);
					PostMessage(hStatusBar, SB_SETTEXT, 
					(WPARAM) (5 | (SBT_OWNERDRAW| SBT_NOBORDERS) ), (LPARAM)(LPWSTR)strText);
			//		MessageBox(hWndMain, _T("recv"), _T("recv"), MB_OK);*/
					// Song Chenguang test 
//					MessageBox(hWndMain, _T("Enter FD_READ"), _T("FD_READ"), MB_OK);
					int nResult = 0;
				...
				//	recv( sockList[nIndex], szRecvData, 1, 0 );
			#if 1	
					nResult = recv( sockList[nIndex], szRecvData, NETHEAD_SIZE, 0 );
				/*	if (recv( sockList[nIndex], szRecvData, NETHEAD_SIZE, 0 ) == SOCKET_ERROR )
					{
						int nRt =  WSAGetLastError();
						wchar_t strText[20];
						memset(strText, 0, sizeof(strText));
						swprintf(strText, _T("%d"), nRt);
						MessageBox(hWndMain, strText, _T("recv1error"), MB_OK);
						break;
					}*/
				/*	if ( nResult != SOCKET_ERROR )
					{
						wchar_t strText[20];
						memset(strText, 0, sizeof(strText));
						swprintf(strText, _T("%d"), nResult);
						DrawLableText(hBtnRecvParamGain, strText, DEFAULT_LABLE_TEXT_HEIGHT, STATIC_TEXT_COLOR, 
											LABEL_START_X, LABEL_6_START_Y);

						while(nResult < NETHEAD_SIZE)
						{
						    int nSize = NETHEAD_SIZE - nResult;						
							int nRlt = recv( sockList[nIndex], &szRecvData[nResult], nSize, 0 );
							nResult = nResult + nRlt;					
						}
					}*/

					// Song Chenguang test
//					MessageBox(hWndMain, _T("After Recv1"), _T("FD_READ"), MB_OK);
			...
			/*		wchar_t strText[20];
					...
					DrawLableText(hBtnRecvParamGain, strText, DEFAULT_LABLE_TEXT_HEIGHT, STATIC_TEXT_COLOR, 
										LABEL_START_X, LABEL_6_START_Y);
					memset(strText, 0, sizeof(strText));
					swprintf(strText, _T("%d"), stPacketHead.wChannelIndex);
					DrawLableText(hBtnRecvParamReferenceGain, strText, DEFAULT_LABLE_TEXT_HEIGHT, STATIC_TEXT_COLOR, 
										LABEL_START_X, LABEL_6_START_Y);
					memset(strText, 0, sizeof(strText));
					swprintf(strText, _T("%d"), stPacketHead.dwSize);
					DrawLableText(hBtnRecvParamDelay, strText, DEFAULT_LABLE_TEXT_HEIGHT, STATIC_TEXT_COLOR, 
										LABEL_START_X, LABEL_6_START_Y);
			*/		
					
				/*	if (recv( sockList[nIndex], &szRecvData[NETHEAD_SIZE], stPacketHead.dwSize, 0 ) == SOCKET_ERROR )
					{
						int nRt =  WSAGetLastError();
						wchar_t strText[20];
						memset(strText, 0, sizeof(strText));
						swprintf(strText, _T("%d"), nRt);
						MessageBox(hWndMain, strText, strText, MB_OK);
						MessageBox(hWndMain, _T("recv2error"), _T("recv2error"), MB_OK);
						break;
					}*/
				...
				/*	if ( nResult != SOCKET_ERROR )
					{
						...
						while(nResult < stPacketHead.dwSize)
						{
						    int nSize = stPacketHead.dwSize - nResult;
							int nRlt = recv( sockList[nIndex], &szRecvData[nResult], nSize, 0 );
							nResult = nResult + nRlt;					
						}
					}*/
					...
					// Song Chenguang test
//					MessageBox(hWndMain, _T("After Recv2"), _T("FD_READ"), MB_OK);
			...
			case FD_CLOSE:
				{
				//	PostMessage(hStatusBar, SB_SETTEXT, 
				//		(WPARAM) (5 | (SBT_OWNERDRAW| SBT_NOBORDERS) ), (LPARAM)(LPWSTR)_T("close"));
...
//	addr.sin_port= htons( 8888 ); // 保证字节顺序
	addr.sin_addr.s_addr= htonl( INADDR_ANY );
		...
		switch(stNetworkEvents.lNetworkEvents)
		{
			case FD_ACCEPT://服务器端接收到连接请求后
				{	
					wchar_t strText[20];
					memset(strText, 0, sizeof(strText));
					swprintf(strText, _T("accept2%d"), nIndex);
					PostMessage(hStatusBar, SB_SETTEXT, 
						(WPARAM) (5 | (SBT_OWNERDRAW| SBT_NOBORDERS) ), (LPARAM)(LPWSTR)strText);
				//	MessageBox(hWndMain, _T("accept"), _T("accept"), MB_OK);
					...
					{  
						PostMessage(hStatusBar, SB_SETTEXT, 
						(WPARAM) (5 | (SBT_OWNERDRAW| SBT_NOBORDERS) ), (LPARAM)(LPWSTR)_T("getSockFail")); 
					}  
					...
					{   
						PostMessage(hStatusBar, SB_SETTEXT, 
						(WPARAM) (5 | (SBT_OWNERDRAW| SBT_NOBORDERS) ), (LPARAM)(LPWSTR)_T("setsockFail")); 
					}  
			...
			case FD_CLOSE:
				...
				//		(WPARAM) (5 | (SBT_OWNERDRAW| SBT_NOBORDERS) ), (LPARAM)(LPWSTR)_T("close"));
					if(gSockClient == sockList[nIndex])
					{
						...
						gnSendCount = 0;
						PostMessage(hStatusBar, SB_SETTEXT, 
							(WPARAM) (5 | (SBT_OWNERDRAW| SBT_NOBORDERS) ), (LPARAM)(LPWSTR)_T("close21"));
					}
					if(gSockClient2 == sockList[nIndex])
					{
						gbChannelSend[1] = FALSE;
						gnSendCount = 0;
						PostMessage(hStatusBar, SB_SETTEXT, 
							(WPARAM) (5 | (SBT_OWNERDRAW| SBT_NOBORDERS) ), (LPARAM)(LPWSTR)_T("close22"));
					}
					if ( stNetworkEvents.iErrorCode[FD_READ_BIT] != 0 )  // 发生网络错误
					{
					 	break;
...
// 				wchar_t strText[20];
// 				memset(strText, 0, sizeof(strText));
// 				wsprintf(strText, _T("wSocketNum = %d"),wSocketNum);
// 				MessageBox(NULL, strText, _T("COMMAND_WAVE_REQ"), MB_OK);
				//////////////////////////////////////////////////////////////////////////
				if(wSocketNum == 0)
				{
				...
				//		PostMessage(hStatusBar, SB_SETTEXT, 
				//			(WPARAM) (5 | (SBT_OWNERDRAW| SBT_NOBORDERS) ), (LPARAM)(LPWSTR)_T("gSockClient"));
					}
				...
				//		PostMessage(hStatusBar, SB_SETTEXT, 
				//			(WPARAM) (5 | (SBT_OWNERDRAW| SBT_NOBORDERS) ), (LPARAM)(LPWSTR)_T("gSockClient2"));
					}
			...
			//	gbChannelSend[stPacketHead.wChannelIndex] = TRUE;
		...
		//		PostMessage(hStatusBar, SB_SETTEXT, 
		//			(WPARAM) (5 | (SBT_OWNERDRAW| SBT_NOBORDERS) ), (LPARAM)(LPWSTR)_T("波形数据请求"));
			}
			break;
	/*	case COMMAND_PEAK_REQ:		//通道伤时伤幅数据请求
			{
			}
			break;
		case COMMAND_DAC:		//DAC曲线
			{
			}
			break;*/
		...
		case COMMAND_ALL_PARAM_SET:	// 设置参数SETONE
			{
				...
				// 关联时由上位机分通道发两个SETONE下来，两通道增益不同,通道2的增益=基准增益+参考增益
			//	SetOne(stSetOne, stPacketHead.wChannelIndex);
			//	memcpy(&gstSetOne[CurrentChannelNo],&stSetOne, stPacketHead.dwSize);			
				RequestRespond(sockClient, pData, stPacketHead.dwSize, bFlag);				
			}
		...
		case COMMAND_WORK_MODE:
			{	
				...
				/*	if(gstSetOne[0].wImpedance == 0)
					{
						wRelay &= 0xFFFC;
						cscan_sys_reg->wRelay = wRelay;							
					}
					else if(gstSetOne[0].wImpedance == 1)
					{
						wRelay &= 0xFFFC;
						wRelay |= 0x1;
						cscan_sys_reg->wRelay = wRelay;
				...
				/*	if(gstSetOne[1].wImpedance == 0)
					{
						wRelay &= 0xFFFC;
						sys_reg->wRelay = wRelay;							
					}
					else if(gstSetOne[1].wImpedance == 2)
					{
						wRelay &= 0xFFFC;
						wRelay |= 0x2;
						cscan_sys_reg->wRelay = wRelay;
...
void SetOne(CSCAN_ARS200FPGA_SETONE stSetOne , WORD wChannelNo)
{
	...
	SetImpedance(stSetOne.wImpedance,wChannelNo);
	SetJianBoMode(stSetOne.stConfig.wDetectMode,wChannelNo);
	if(wChannelNo == 0)
	{
...
void SetProbeBand(WORD wBand, WORD wChannelNo)
{
	...
	if(!gbGuanLian)
	{
		...
		if(wChannelNo == 0)
		{
			wMax500Value = (2<<8) + wLvBo;
			send_max500(wMax500Value);
		}
		else if(wChannelNo == 1)
		{
			wMax500Value = (3<<8) + wLvBo;
			send_max500(wMax500Value);
		}
	}
	else
...
void SetJianBoMode(WORD wJianBoMode, WORD wChannelNo)
{
	...
	if(!gbGuanLian)
	{
		if(CurrentChannelNo == 0)
		{
			cscan_ch1_config_reg->wDetectMode = wJianBoMode;
		}
		else if(CurrentChannelNo == 1)
		{
			cscan_ch2_config_reg->wDetectMode = wJianBoMode;
		}
	}
	else
	{
