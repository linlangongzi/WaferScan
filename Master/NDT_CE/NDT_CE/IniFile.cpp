...
//	LPCTSTR szValue = _T(""),
	int ret = CeGetPrivateProfileString(lpAppName, lpKeyName, _T("defaultValue"), szValue, 0, lpFileName);
	if(wcscmp(szValue, _T("defaultValue")) == 0)
	{
		return nDefault;
	}
	else
...
//读取一个float值
float CeGetPrivateProfileFloat(
							LPCTSTR lpAppName,
							LPCTSTR lpKeyName,
							float fDefault,
							LPCTSTR lpFileName
...
//	wchar_t* szValue = _T("");
	...
	else
	{
		if(ret > 0)
		{
			char szTemp[MAX_PATH]; 
			BOOL bRet = FALSE;
			WideCharToMultiByte(CP_ACP, 0, szValue,
				-1, szTemp, MAX_PATH, NULL, NULL);
