...
BOOL CSimpleFree::InitItemsValue()
{
	...
	m_Items[0].szItemID = L"PLCState";//读 电气状态
	m_Items[0].vtRequestedDataType = VT_UI1;
	m_Items[1].szItemID = L"WSState";//读 仪器状态  （好像没用）
	m_Items[1].vtRequestedDataType = VT_UI1;
	m_Items[2].szItemID = L"PipeLength";//写 钢管长度
	m_Items[2].vtRequestedDataType = VT_UI4;
	m_Items[3].szItemID = L"PipeDiameter";//写 钢管直径
	m_Items[3].vtRequestedDataType = VT_UI4;
	m_Items[4].szItemID = L"PipeTestResult";//写 探伤结果
	...
	LPWSTR strProbeFlawFlag[25] = { 
		...
		L"ProbeWallFlawFlag1", L"ProbeWallFlawFlag2",L"ProbeWallFlawFlag3", L"ProbeWallFlawFlag4",L"ProbeWallFlawFlag5" };//写各通道探伤结果
