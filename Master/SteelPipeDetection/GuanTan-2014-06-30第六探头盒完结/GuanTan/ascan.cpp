...
void AScan::checkBoxProbe6Change()	//探头6 测厚 开关函数
{
	...
	else
	{
		m_nProbe = 6;
		m_nFlawOrBottom = 0;
		m_wMapChannelNum = 31 + m_nProbeBox;
		ChangeMapCPicData(m_wMapChannelNum);
		UpdateProbe();
		ProbeRemember();
	}
...
void AScan::GateIPainter()
{
	if (m_bGateI)
	{
		...
		if (m_bGateIIsPressed==true)
		{
			...
			char cXLocation[10];
			//编码器值
			float tempX = (float)(m_CH_FLAWDATA_CSCAN_D.wXHigh16 * 65536  + m_CH_FLAWDATA_CSCAN_D.wXLow16) / (X_TO_LENGTH);
			if (m_nProbeBox == 0)
			{
...
void AScan::SetProbeInfo(void)
{
	if (m_nFlawOrBottom == 0)
	{
		m_fDelay = m_ChannelInfo[m_MapCONFIG_Channel[m_wChannelNo + 6]].m_fChannelDelay;
		int nMin = Dot2Range(m_ChannelInfo[m_MapCONFIG_Channel[m_wChannelNo + 6]].m_nWaveSpeed, MONITOR_SIZE);
		if (m_ChannelInfo[m_MapCONFIG_Channel[m_wChannelNo + 6]].m_fChannelRange < nMin)
		{
			m_ChannelInfo[m_MapCONFIG_Channel[m_wChannelNo + 6]].m_fChannelRange = nMin;
		}
		m_fRange = m_ChannelInfo[m_MapCONFIG_Channel[m_wChannelNo + 6]].m_fChannelRange;
		m_fCGain = m_ChannelInfo[m_MapCONFIG_Channel[m_wChannelNo + 6]].m_fChannelCPlus;
		m_fGain = m_ChannelInfo[m_MapCONFIG_Channel[m_wChannelNo + 6]].m_fChannelPlus;
		//更新抑制
		m_fSuppression = m_ChannelInfo[m_MapCONFIG_Channel[m_wChannelNo + 6]].m_fRestrain;
		//更新重复频率
		m_fReFreq = m_ChannelInfo[m_MapCONFIG_Channel[m_wChannelNo + 6]].m_ProbeInfo.m_wReFreq;
		DWORD dwDelay = MicroSecond2Dot(m_fDelay);
	...
	else
	{
		...
		if(event->timerId()==m_nTIMER_Sel_Channel)
		{
			...
			for (itPicData = m_MapCPicData.begin();itPicData != m_MapCPicData.end(); itPicData++)
			{
				...
				{
					if (itPicData->first > 30)
					{
						g_dwActiveChannel = itPicData->first - 6;
...
{
	//添加第六组探头盒，重新编排对应方式 wangwei modified on 2014-6-13
	for (int i = 0;i < 30;i++)
	{
