...
#define DRAW_BSCAN_DELAY_1 122
#define DRAW_BSCAN_DELAY_2 121
#define DRAW_BSCAN_DELAY_3 120
#define DRAW_BSCAN_BEGIN 113
#define TEMP_DATA_COUNT 50
#define AVERAGE_NUM 20
#define AVERAGE_LOW_NUM 5
#define TEST_PERSENT 0.2
#define PI 3.141592653589793

#define HAVE_OPC			1	// 是否包含OPC：0不包含；1包含
#define SAVE_BOX_COUPLE		0	// 保存耦合不良原始数据
#define SAVE_BOX_THICK		0	// 保存原始测厚数据
#define SAVE_PROBE_FEATURE	0	// 存储缺陷探头原始数据
...
public:
	...
	void CalcFlaw(VectorPoint vecPoint,int& nFlawNum,int& nMaxFlawSize,float& fMaxFlawHeight);	//缺陷统计-适用于 内外部，横纵向
	void CalcBadCouple(VectorPoint vecPoint,int& nFlawNum,int& nMaxFlawSize);					//缺陷统计-适用于耦合不良
	void CalcThick(VectorPoint vecPointHigh,VectorPoint vecPointLow);						//缺陷统计-适用于壁厚测量
	void CalcThick2(VectorPoint vecPointHigh, VectorPoint vecPointLow);						//缺陷统计-适用于壁厚测量
...
private:
	...
	void SaveData(QString fileName,VectorPoint ptVector);	//保存各缺陷扫描原始数据
...
private:
	...
	BOOL m_bEncipherorChange6;//编码器值变了10以上
...
// 测厚新算法
	QMap<int, VectorPoint> m_featureMap;
	QMap<int, VectorPoint> m_OutHMap;	// 探伤降噪Map
	QMap<int, VectorPoint> m_OutVMap;	// 探伤降噪Map
	QMap<int, VectorPoint> m_InHMap;	// 探伤降噪Map
	QMap<int, VectorPoint> m_InVMap;	// 探伤降噪Map
	VectorPoint m_vectorOutVFeature;		//外部纵向缺陷曲线点集
	VectorPoint m_vectorInVFeature;			//内部纵向缺陷曲线点集
	VectorPoint m_vectorOutHFeature;		//外部横向缺陷曲线点集
	VectorPoint m_vectorInHFeature;			//内部横向缺陷曲线点集
	VectorPoint m_vectorThickHighFeature;	//测厚-增厚缺陷厚面曲线点集
	VectorPoint m_vectorThickLowFeature;	//测厚-减薄缺陷厚面曲线点集
	VectorPoint m_vectorLayerFeature;		//分层缺陷曲线点集
	VectorPoint m_vectorBadCoupleFeature;	//耦合不良曲线点集
	
// 测厚旧算法
	VectorPoint m_vectorThickHighFeature2;	//测厚-增厚缺陷厚面曲线点集
	VectorPoint m_vectorThickLowFeature2;	//测厚-减薄缺陷厚面曲线点集
	VectorPoint m_vectorThickFeature1;		//测厚-平均壁厚曲线点集
	VectorPoint m_vectorThickFeature2;		//测厚-平均壁厚曲线点集
	VectorPoint m_vectorThickFeature3;		//测厚-平均壁厚曲线点集
	VectorPoint m_vectorThickFeature4;		//测厚-平均壁厚曲线点集
	VectorPoint m_vectorThickFeature5;		//测厚-平均壁厚曲线点集
	VectorPoint m_vectorThickFeature6;		//测厚-平均壁厚曲线点集
	VectorPoint m_vectorThickTempFeature1;	//测厚-探头盒1-10mm内所有点临时存储
	VectorPoint m_vectorThickTempFeature2;	//测厚-探头盒2-10mm内所有点临时存储
	VectorPoint m_vectorThickTempFeature3;	//测厚-探头盒3-10mm内所有点临时存储
	VectorPoint m_vectorThickTempFeature4;	//测厚-探头盒4-10mm内所有点临时存储
	VectorPoint m_vectorThickTempFeature5;	//测厚-探头盒5-10mm内所有点临时存储
	VectorPoint m_vectorThickTempFeature6;	//测厚-探头盒5-10mm内所有点临时存储

	VectorPoint m_vecBadCoupleBox1;
	VectorPoint m_vecBadCoupleBox2;
	VectorPoint m_vecBadCoupleBox3;
	VectorPoint m_vecBadCoupleBox4;
	VectorPoint m_vecBadCoupleBox5;
	VectorPoint m_vecBadCoupleBox6;

#if SAVE_BOX_THICK
	VectorPoint m_vectorFeature1;	// 原始数据存贮
	VectorPoint m_vectorFeature2;
	VectorPoint m_vectorFeature3;
	VectorPoint m_vectorFeature4;
	VectorPoint m_vectorFeature5;
	VectorPoint m_vectorFeature6;
#endif

#if SAVE_PROBE_FEATURE
	VectorPoint m_vectorProbe1;	// 缺陷探头数据存储
	VectorPoint m_vectorProbe2;	// 缺陷探头数据存储
	VectorPoint m_vectorProbe3;	// 缺陷探头数据存储
	VectorPoint m_vectorProbe4;	// 缺陷探头数据存储
	VectorPoint m_vectorProbe5;	// 缺陷探头数据存储
	VectorPoint m_vectorProbe6;	// 缺陷探头数据存储
	VectorPoint m_vectorProbe7;	// 缺陷探头数据存储
	VectorPoint m_vectorProbe8;	// 缺陷探头数据存储
	VectorPoint m_vectorProbe9;	// 缺陷探头数据存储
	VectorPoint m_vectorProbe10;	// 缺陷探头数据存储
	VectorPoint m_vectorProbe11;	// 缺陷探头数据存储
	VectorPoint m_vectorProbe12;	// 缺陷探头数据存储
	VectorPoint m_vectorProbe13;	// 缺陷探头数据存储
	VectorPoint m_vectorProbe14;	// 缺陷探头数据存储
	VectorPoint m_vectorProbe15;	// 缺陷探头数据存储
	VectorPoint m_vectorProbe16;	// 缺陷探头数据存储
	VectorPoint m_vectorProbe17;	// 缺陷探头数据存储
	VectorPoint m_vectorProbe18;	// 缺陷探头数据存储
	VectorPoint m_vectorProbe19;	// 缺陷探头数据存储
	VectorPoint m_vectorProbe20;	// 缺陷探头数据存储
	VectorPoint m_vectorProbe21;	// 缺陷探头数据存储
	VectorPoint m_vectorProbe22;	// 缺陷探头数据存储
	VectorPoint m_vectorProbe23;	// 缺陷探头数据存储
	VectorPoint m_vectorProbe24;	// 缺陷探头数据存储
#endif

private:
	int m_nCutNoise;		// 横伤
	int m_nCutNoise2;		// 纵伤
	int m_nThickDrawType;	// 1:新算法  2:旧算法 3:两者
	int m_nfeatureAverageHighCount;
	int m_nfeatureAverageLowCount;
	...
	int m_nThickFeatureFlawNumAdd2;		//壁厚-增厚缺陷数量
	int m_nThickFeatureFlawNumCut2;		//壁厚-减薄缺陷数量
	...
	int m_nThickFeatureMaxFlawSizeAdd2;	//壁厚-增厚最大缺陷尺寸2
	int m_nThickFeatureMaxFlawSizeCut2;	//壁厚-减薄最大缺陷尺寸2
	int m_nLayerFeatureMaxFlawSize;		//分层缺陷最大缺陷尺寸
	int m_nBadCoupleFeatureMaxFlawSize;	//耦合不良最大缺陷尺寸
	int m_nBadCoupleBox[6];				//探头盒耦合不良计数数组
	int m_nBadCoupleNum;				//离线查看耦合不良计数
	int m_nBadCoupleBoxNum;				//各盒耦合不良报警值

	float m_fThickPercent;
	...
	float m_fThickFeatureMaxFlawGain2;	//壁厚增厚缺陷最大长度所占百分比2
	float m_fThickFeatureMinFlawGain2;	//壁厚减薄缺陷最大长度所占百分比2
	...
	float m_fProbeCorrect[6];			//5个盒子测厚探头校准
	float m_fHighCorrect;
	float m_fLowCorrect;
	float m_fHighCorrect2;
	float m_fLowCorrect2;
...
private:
	...
	QString m_strSingleReport10;	//壁厚增厚
	QString m_strSingleReport11;	//壁厚减薄
...
private slots:
	void ActionPrintScreen();
	void ThickDrawTypeChanged();
	void Thick1Show();
	void Thick2Show();
	void Thick12Show();
...
//参数设置
	...
	void DefectAlarm(int index, bool alarm);		//缺陷报警
	void CouplingAlarm(int index, bool alarm);	//耦合报警
	void ClearAlarm();
	...
	void StaNoiseCheck1_12();	//统计降噪1-10读取文件
	void StaNoiseCheck13_24();	//统计降噪11-20读取文件
	void StaNoiseCheck25_36();	//统计降噪21-30读取文件
	void GateSetCheck1_12();	//闸门设置1-10读取文件
	void GateSetCheck13_24();	//闸门设置11-20读取文件
	void GateSetCheck25_36();	//闸门设置21-30读取文件
	void JudgeCheck1_12();		//闸门开关设置1-10读取文件
	void JudgeCheck13_24();		//闸门开关设置11-20读取文件
	void JudgeCheck25_36();		//闸门开关设置21-30读取文件
...
public:
	...
	CChannelInfo m_Parameter_[CHANNEL_NUM + 4];
	...
	BOOL m_bSelChannel;
	BOOL *m_bPlateEnd;
	bool m_bDefectAlarm[36];//缺陷报警数组
	bool m_bCouplingAlarm[36];//耦合报警数组
...
private:
	...
	typedef QVector<QLabel *> VecLabel;
	VecLabel vecLabelDefect;
	VecLabel vecLabelCouple;
};
