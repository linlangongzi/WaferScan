...
public:
	...
	WORD m_wChannelNo;
	CChannelInfo m_ChannelInfo[CHANNEL_NUM + 4];//分层测厚每个物理通道分两个软通道，有8个预留物理测厚通道，此版本使用6个测厚
...
public:
	...
	void GateIPainter(QPainter &painter);
	void GateAPainter(QPainter &painter);
	void GateBPainter(QPainter &painter);
	void GateI2Painter(QPainter &painter);
	void GateMoveUpdate();
	void UpdateProbe();
	void CurSelectProbe();
	void ProbeRemember();
...
private slots:
	...
	void checkBoxProbe4Change();		//探头4开关函数
	void checkBoxProbe5Change();		//探头5开关函数
	void checkBoxProbe6Change();		//探头6开关函数
...
private:
	...
	void GetScreenInfo();
	void keyPressEvent(QKeyEvent *event);//重写esc响应
