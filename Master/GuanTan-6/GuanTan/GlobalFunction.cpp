...
extern QVector<int> g_vecADataIndex;
extern CH_FLAWDATA_CSCAN_D g_CH_FLAWDATA_CSCAN_D;
DataMonitor g_DataMonitor;
extern WORD g_wStartFlag;			//探伤使能；0，为结束；1，为开始
...
extern QMutex g_startMarkMutex;
...
*/
	WORD wValue = 1;
	P9054_WriteWord(pThreadParam->hDev, P9054_ADDR_SPACE0, SYS_RESET_ADDRESS, wValue);	// 系统复位信号地址

	wValue = 0;
	P9054_WriteWord(pThreadParam->hDev, P9054_ADDR_SPACE0, SYS_RESET_ADDRESS, wValue);

	wValue = 1;
	P9054_WriteWord(pThreadParam->hDev, P9054_ADDR_SPACE0, FEATURE_BUFFER_RESET_ADDRESS, wValue);	// 特征缓冲区数据复位命令地址

	wValue = 0;
	P9054_WriteWord(pThreadParam->hDev, P9054_ADDR_SPACE0, FEATURE_BUFFER_RESET_ADDRESS, wValue);

	wValue = 1;
	P9054_WriteWord(pThreadParam->hDev, P9054_ADDR_SPACE0, MONITOR_DATA_RESET_ADDRESS, wValue);	// 监视数据复位命令地址
...
int AECThread_Idle(AECTHREAD_PARAM * pThreadParam)
{
	...
	if (g_dwActiveChannel / PER_CARD_CHANNEL_NUM == iCardIndex) {		
		...
		for (int i = 0; i < g_vecADataIndex.count(); ++i) {
			if (wTempChannelNum == g_vecADataIndex.value(i)) {
				bExist = TRUE;
			}
		}
		...
		if (bExist) {
			...
			if(bRet) {
				g_DataMonitor.SendADataReceived();
			}
		}

		::ZeroMemory(&g_CH_FLAWDATA_CSCAN_D, sizeof(CH_FLAWDATA_CSCAN_D));

		g_CH_FLAWDATA_CSCAN_D.wXHigh16 = P9054_ReadWord(pThreadParam->hDev, P9054_ADDR_SPACE0, MONITOR_FEATURE_DATA_ADDRESS + 0x14);
		g_CH_FLAWDATA_CSCAN_D.wXLow16 = P9054_ReadWord(pThreadParam->hDev, P9054_ADDR_SPACE0, MONITOR_FEATURE_DATA_ADDRESS + 0x12);
		g_CH_FLAWDATA_CSCAN_D.wGate1SyncTime = P9054_ReadWord(pThreadParam->hDev, P9054_ADDR_SPACE0, MONITOR_FEATURE_DATA_ADDRESS);
		g_CH_FLAWDATA_CSCAN_D.wGate1MPAmp = P9054_ReadWord(pThreadParam->hDev, P9054_ADDR_SPACE0, MONITOR_FEATURE_DATA_ADDRESS + 0x2);
		g_CH_FLAWDATA_CSCAN_D.wGate2MPTime = P9054_ReadWord(pThreadParam->hDev, P9054_ADDR_SPACE0, MONITOR_FEATURE_DATA_ADDRESS + 0x4);
		g_CH_FLAWDATA_CSCAN_D.wGate2MPAmp = P9054_ReadWord(pThreadParam->hDev, P9054_ADDR_SPACE0, MONITOR_FEATURE_DATA_ADDRESS + 0x6);
		g_CH_FLAWDATA_CSCAN_D.wGate3MPTime = P9054_ReadWord(pThreadParam->hDev, P9054_ADDR_SPACE0, MONITOR_FEATURE_DATA_ADDRESS + 0x8);
		g_CH_FLAWDATA_CSCAN_D.wGate3MPAmp = P9054_ReadWord(pThreadParam->hDev, P9054_ADDR_SPACE0, MONITOR_FEATURE_DATA_ADDRESS + 0xA);
		g_CH_FLAWDATA_CSCAN_D.wGate4SyncTime = P9054_ReadWord(pThreadParam->hDev, P9054_ADDR_SPACE0, MONITOR_FEATURE_DATA_ADDRESS + 0xC);
		g_CH_FLAWDATA_CSCAN_D.wGate4MPAmp = P9054_ReadWord(pThreadParam->hDev, P9054_ADDR_SPACE0, MONITOR_FEATURE_DATA_ADDRESS + 0xE);

		g_DataMonitor.SendAScanCDataReceived();
	...
	// 读取伤数据
	if (g_startMarkMutex.tryLock())
	{
		WORD startMark = g_wStartFlag;
		g_startMarkMutex.unlock();

		if (startMark) {
			BOOL bRet = ReadFlawResult(pThreadParam, iCardIndex);
			if (bRet) {
				g_DataMonitor.SendCDataReceived();
			}
		}
	}
...
// 		int nSize = sizeof(FEATURE_DATA) /  sizeof(WORD) * 2 * DATA_NUM;
// 		P9054_DMAReadWriteBlock(p->hDev, dwAddress, g_stFeatrueInfo, nSize, TRUE, P9054_MODE_WORD, P9054_DMA_CHANNEL_1);
