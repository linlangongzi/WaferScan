...
class CMemDCForRuler : public CDC
{
public:

    //constructor sets up the memory DC
    CMemDCForRuler(CDC* pDC) : CDC()
    {
        ASSERT(pDC != NULL);
    ...
    ~CMemDCForRuler()
    {
        if (m_bMemDC)
        {
			...
			DeleteObject(m_bitmap);		// Song Chenguang add on 2013-07-04
        } else {
    ...
    CMemDCForRuler* operator->() {return this;}
        
    // Allow usage as a pointer
    operator CMemDCForRuler*() {return this;}
