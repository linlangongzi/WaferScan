...
*  File: plx_diag.c
*
*  Sample user-mode diagnostics application for accessing PLX
...
#define PLX_CFG_REGS_NUM sizeof(gPLX_CfgRegs) / sizeof(WDC_REG)
/* TODO: For read-only or write-only registers, change the direction field of
         the relevant registers in gPLX_CfgRegs to WDC_READ or WDC_WRITE. */
/* NOTE: You can define additional configuration registers in gPLX_CfgRegs. */
const WDC_REG *gpPLX_CfgRegs = gPLX_CfgRegs;

/* -----------------------------------------------
    PLX run-time registers information
   ----------------------------------------------- */
/* Run-time registers information array */
/* const WDC_REG gPLX_Regs[]; */
const WDC_REG *gpPLX_Regs = NULL;
/* TODO: You can remove the comment from the gPLX_Regs array declaration and
         fill the array with run-time registers information for your device,
         in which case be sure to set gpPLX_Regs to point to gPLX_Regs. */
#define PLX_REGS_NUM 0
...
int main(void)
{
    ...
    {
        PLX_ERR("plx_diag: Failed to initialize the PLX library: %s",
            PLX_GetLastErr());
        return dwStatus;
    }

    /* Find and open a PLX device (by default ID) */
    if (PLX_DEFAULT_VENDOR_ID)
        hDev = DeviceFindAndOpen(PLX_DEFAULT_VENDOR_ID, PLX_DEFAULT_DEVICE_ID);
    ...
    if (WD_STATUS_SUCCESS != dwStatus)
        PLX_ERR("plx_diag: Failed to uninit the PLX library: %s", PLX_GetLastErr());
...
//            break;
//        case MENU_MAIN_RW_CFG_SPACE: /* Read/Write the PLX configuration space */
...
static BOOL DeviceFind(DWORD dwVendorId, DWORD dwDeviceId, WD_PCI_SLOT *pSlot)
{
    ...
    if (WD_STATUS_SUCCESS != dwStatus)
    {
        PLX_ERR("DeviceFind: Failed scanning the PCI bus.\n"
    ...
    if (!dwNumDevices)
    {
        PLX_ERR("No matching device was found for search criteria "
...
static WDC_DEVICE_HANDLE DeviceOpen(const WD_PCI_SLOT *pSlot)
{
    ...
    if (WD_STATUS_SUCCESS != dwStatus)
    {
        PLX_ERR("DeviceOpen: Failed retrieving the device's resources information.\n"
            "Error 0x%lx - %s\n", dwStatus, Stat2Str(dwStatus));
        return NULL;
    }
    ...
    hDev = PLX_DeviceOpen(&deviceInfo);
    if (!hDev)
    {
        PLX_ERR("DeviceOpen: Failed opening a handle to the device: %s",
            PLX_GetLastErr());
        return NULL;
    }
...
static void DeviceClose(WDC_DEVICE_HANDLE hDev)
{
    ...
    if (!PLX_DeviceClose(hDev))
    {
        PLX_ERR("DeviceClose: Failed closing PLX device: %s",
            PLX_GetLastErr());
    }
}
...
static void MenuReadWriteAddr(WDC_DEVICE_HANDLE hDev)
{
    ...
    if (ACTIVE_ADDR_SPACE_NEEDS_INIT == dwAddrSpace)
    {
        ...
        {
            PLX_ERR("MenuReadWriteAddr: Error - no active address spaces found\n");
...
static void SetAddrSpace(WDC_DEVICE_HANDLE hDev, PDWORD pdwAddrSpace)
{
    ...
    for (dwAddrSpace = 0; dwAddrSpace < dwNumAddrSpaces; dwAddrSpace++)
    {
        ...
        {
            PLX_ERR("SetAddrSpace: Error - Failed to get address space information: %s",
                PLX_GetLastErr());
            return;
        }
...
static void MenuReadWriteCfgSpace(WDC_DEVICE_HANDLE hDev)
{
    ...
    do {
        ...
        if (PLX_CFG_REGS_NUM)
        {
            ...
            printf("------------------------\n");
            WDC_DIAG_RegsInfoPrint(gpPLX_CfgRegs, PLX_CFG_REGS_NUM,
        ...
        if (DIAG_INPUT_FAIL == DIAG_GetMenuOption(&option,
            PLX_CFG_REGS_NUM ? MENU_RW_CFG_SPACE_WRITE_REG :
            MENU_RW_CFG_SPACE_WRITE_OFFSET))
        {
            continue;
        }
        ...
        case MENU_RW_CFG_SPACE_READ_ALL_REGS:
            WDC_DIAG_ReadRegsAll(hDev, gpPLX_CfgRegs, PLX_CFG_REGS_NUM, TRUE);
            break;
        case MENU_RW_CFG_SPACE_READ_REG:  /* Read from a configuration register */
            WDC_DIAG_ReadWriteReg(hDev, gpPLX_CfgRegs, PLX_CFG_REGS_NUM, WDC_READ, TRUE);
            break;
        case MENU_RW_CFG_SPACE_WRITE_REG: /* Write to a configuration register */
            WDC_DIAG_ReadWriteReg(hDev, gpPLX_CfgRegs, PLX_CFG_REGS_NUM, WDC_WRITE, TRUE);
            break;
        }
...
static void MenuReadWriteRegs(WDC_DEVICE_HANDLE hDev)
{
    ...
    do {
        ...
        printf("--------------------------\n");   
        WDC_DIAG_RegsInfoPrint(gpPLX_Regs, PLX_REGS_NUM, WDC_DIAG_REG_PRINT_ALL);
        ...
        case MENU_RW_REGS_READ_ALL:
            WDC_DIAG_ReadRegsAll(hDev, gpPLX_Regs, PLX_REGS_NUM, FALSE);
            break;
        case MENU_RW_REGS_READ_REG:  /* Read from a register */
            WDC_DIAG_ReadWriteReg(hDev, gpPLX_Regs, PLX_REGS_NUM, WDC_READ, FALSE);
            break;
        case MENU_RW_REGS_WRITE_REG: /* Write to a register */
            WDC_DIAG_ReadWriteReg(hDev, gpPLX_Regs, PLX_REGS_NUM, WDC_WRITE, FALSE);
            break;
        }
...
static void MenuInterrupts(WDC_DEVICE_HANDLE hDev)
{
    ...
    /* TODO: You can remove this message after you have modified the
             implementation of PLX_IntEnable() in plx_lib.c to correctly
             acknowledge the interrupts (see guidelines in PLX_IntEnable()) */
    printf("\n");
    printf("WARNING!!!\n");
    printf("----------\n");
    ...
    printf("Before enabling the interrupts, you must first modify the source code\n"
        "of PLX_IntEnable(), in the file plx_lib.c, to correctly acknowledge\n"
        ...
        case MENU_INT_ENABLE_DISABLE: /* Enable/disable interrupts */
            if (fIntEnable)
            {
                if (WD_STATUS_SUCCESS == PLX_IntEnable(hDev, DiagIntHandler))
                    ...
                    PLX_ERR("Failed enabling interrupts: %s", PLX_GetLastErr());
            }
            else
            {
                if (WD_STATUS_SUCCESS == PLX_IntDisable(hDev))
                    ...
                    PLX_ERR("Failed disabling interrupts: %s", PLX_GetLastErr());
            }
            break;
        }
...
static void MenuEvents(WDC_DEVICE_HANDLE hDev)
{
    ...
    do
    {
        ...
        case MENU_EVENTS_REGISTER_UNREGISTER: /* Register/unregister events */
            if (fRegister)
            {
                if (WD_STATUS_SUCCESS == PLX_EventRegister(hDev, DiagEventHandler))
                    printf("Events registered\n");
                else
                    PLX_ERR("Failed to register events. Last error:\n%s", PLX_GetLastErr());
            }
            else
            {
                if (WD_STATUS_SUCCESS == PLX_EventUnregister(hDev))
                    ...
                    PLX_ERR("Failed to unregister events. Last Error:\n%s", PLX_GetLastErr());
            }
            break;
        }
