...
: QDialog(parent, flags)
{
	...
	m_MapCPicData[m_wChannelNo] = pCPicData;
...
void AScan::DataMemberInit()	//成员变量初始化
{
	...
	m_bGateIIsPressed=true;
	m_fGateILeftUpdate=0.05;
	m_fGateITopUpdate=0.02;
	m_bGateAIsPressed=false;
	m_fGateALeftUpdate=0.05;
	m_fGateATopUpdate=0.05;
	m_bGateBIsPressed=false;
	m_fGateBLeftUpdate=0.05;
	m_fGateBTopUpdate=0.08;
	m_bGateI2IsPressed=false;
	m_fGateI2LeftUpdate=0.05;
	m_fGateI2TopUpdate=0.11;
	m_bGateIIsPresse=false;
	m_bGateILIsPresse=false;
	m_bGateIRIsPresse=false;
	m_bGateAIsPresse=false;
	...
	for (int i = 0;i < 47;i++)
	{
		...
		{	
			if (i == 0 && j == 0)
				m_bProbeOnOrOff[i][j] = 1;
			else
				m_bProbeOnOrOff[i][j] = 0;
		}
	}
	emit AscanInfoChanged();
}
...
void AScan::UiInit()	//Ui初始化
{
	...
	connect(spinBoxReFreq,SIGNAL(valueChanged(double)),this,SLOT(slotSpinbox_SliderReFreq()));
	connect(horizontalSliderReFreq,SIGNAL(valueChanged(int)),this,SLOT(slotSlider_SpinboxReFreq()));
	connect(spinBoxReFreq,SIGNAL(valueChanged(double)),this,SLOT(ReFreqChange()));
	horizontalSliderReFreq->setRange(MIN_RE_FREQ,MAX_RE_FREQ);
	spinBoxReFreq->setRange((double)MIN_RE_FREQ,(double)MAX_RE_FREQ);
	...
	for (int i = 1;i <= 45;i++)
	{
		...
		{
			QString str = tr("内探2●探头盒") + QString::number(i-23);
			comboBoxProbeBox->addItem(str);
		}
	}
	UpdateProbe();
...
void AScan::paintEvent(QPaintEvent *e)
{
	...
	painter.end();
	DrawGrid(m_rectWave,m_fDelayOut,m_nGrid,m_nLines);
	DrawWave(m_rectWave);
	GateIPainter();
	GateAPainter();
	GateBPainter();
	...
	painter.end();
	DrawVRuler(m_rectVRuler,0,100,5,1,0);
	DrawHRuler(m_rectHRuler,(int)m_fXMinRuler,(int)m_fXMaxRuler,(int)(m_fXMaxRuler-m_fXMinRuler)/10,0,0);
...
void AScan::DrawGrid(const QRect& rect,double dXMove,int *nGrid,int nlines)	//画网格函数
{
	QPainter painter(this);
...
void AScan::DrawHRuler(const QRect& rect,int nStart, int nEnd, int nStep, int nTextLocation,int nTextDirection)	//画水平标尺函数 
{
...
void AScan::DrawVRuler(const QRect& rect,int nStart, int nEnd, int nStep, int nTextLocation,int nTextDirection)	//画纵向标尺函数
{
	QPainter painter(this);
	painter.setPen(Qt::black);
	if (nEnd == 20)
...
void AScan::DrawWave(const QRect& rect)	//画标尺顶角方块
{
	QPainter painter(this);
...
void AScan::SetAScanData(WORD wChannelNum,int nFlawOrBottom,CPicData* pPicData)
{
	...
	for (itPicData = m_MapCPicData.begin();itPicData != m_MapCPicData.end(); itPicData++)
	{
		if (wChannelNum == itPicData->first)
		{
			bExist = TRUE;
			m_MapCPicData.erase(itPicData++);
			break;
		}
	}
	if (bExist)
	{
		...
		m_MapCPicData[wChannelNum] = pPicData;
...
void AScan::ProbeBoxChange()	//探头盒切换函数
{
	...
	for (int i = 0;i <47;i++)
	{
		if (m_nProbeBox == i && i < 46)
		{
			checkBoxProbe2->setEnabled(true);
			checkBoxProbe3->setEnabled(true);
			if (m_bProbeOnOrOff[i][0] == 0)
				checkBoxProbe1->setChecked(false);
			else
				checkBoxProbe1->setChecked(true);
			if (m_bProbeOnOrOff[i][1] == 0)
				checkBoxProbe2->setChecked(false);
			else
				checkBoxProbe2->setChecked(true);
			if (m_bProbeOnOrOff[i][2] == 0)
		...
		{
			checkBoxProbe2->setEnabled(false);
			checkBoxProbe3->setEnabled(false);
			if (m_bProbeOnOrOff[i][0] == 0)
				checkBoxProbe1->setChecked(false);
			else
				checkBoxProbe1->setChecked(true);
	...
	for (itPicData = m_MapCPicData.begin();itPicData != m_MapCPicData.end(); itPicData++)
	{
		if (itPicData->first < 21)
		{
			WORD wTempChannelNum = itPicData->first + nProbeBoxChange * 4;
            m_MapCPicDataTemp[wTempChannelNum] = itPicData->second;
		}
		else
		{
			WORD wTempChannelNum = itPicData->first + nProbeBoxChange;
			m_MapCPicDataTemp[wTempChannelNum] = itPicData->second;;
		}
	}
    m_MapCPicData.clear();
	...
	{
		m_MapCPicData[itPicData->first] = itPicData->second;;
	}
	SetProbeInfo();
	SetGateInfo();
	UpdateProbe();
...
void AScan::checkBoxProbe1Change()	//探头1开关函数
{
	for (int i = 0;i < 47;i++)
	{
		if (m_nProbeBox == i)
		{
			if (m_bProbeOnOrOff[i][0] == 0)
			{
				m_bProbeOnOrOff[i][0] = 1;
			}
			else
			{
				m_bProbeOnOrOff[i][0] = 0;
			}
		}
	}
	m_nProbe = 1;
...
void AScan::checkBoxProbe2Change()	//探头2开关函数
{
	for (int i = 0;i < 47;i++)
	{
		if (m_nProbeBox == i)
		{
			if (m_bProbeOnOrOff[i][1] == 0)
			{
				m_bProbeOnOrOff[i][1] = 1;
			}
			else
			{
				m_bProbeOnOrOff[i][1] = 0;
			}
		}
	}
	m_nProbe = 2;
...
void AScan::checkBoxProbe3Change()	//探头3开关函数
{
	for (int i = 0;i < 47;i++)
	{
		if (m_nProbeBox == i)
		{
			if (m_bProbeOnOrOff[i][2] == 0)
			{
				m_bProbeOnOrOff[i][2] = 1;
			}
			else
			{
				m_bProbeOnOrOff[i][2] = 0;
			}
		}
	}
	m_nProbe = 3;
...
void AScan::checkBoxGateSwitchChange()	//闸门开关函数
{
	...
	{
		m_ChannelInfo[m_MapCONFIG_Channel[m_wChannelNo+5]].m_GateInfo[0].m_bOpen = m_bGateI;
		m_ChannelInfo[m_MapCONFIG_Channel[m_wChannelNo+5]].m_GateInfo[1].m_bOpen = m_bGateA;
		m_ChannelInfo[m_MapCONFIG_Channel[m_wChannelNo+5]].m_GateInfo[2].m_bOpen = m_bGateB;
		m_ChannelInfo[m_MapCONFIG_Channel[m_wChannelNo+5]].m_GateInfo[3].m_bOpen = m_bGateI2;
	}
	else
	{
...
void AScan::GateFrontChange()	//闸门前沿变量跟踪滑块变化函数
{
	...
	if (m_bGateIIsPressed)
	{
		...
		if (m_nFlawOrBottom == 0)
		{
		      m_ChannelInfo[m_MapCONFIG_Channel[m_wChannelNo+5]].m_GateInfo[0].m_fFrontGate = m_fIGateFront;
		}
		else
		{
	...
	if (m_bGateAIsPressed)
	{
		...
		if (m_nFlawOrBottom == 0)
		{
			m_ChannelInfo[m_MapCONFIG_Channel[m_wChannelNo+5]].m_GateInfo[1].m_fFrontGate = m_fAGateFront;
		}
		else
		{
	...
	if (m_bGateBIsPressed)
	{
		...
		if (m_nFlawOrBottom == 0)
		{
			m_ChannelInfo[m_MapCONFIG_Channel[m_wChannelNo+5]].m_GateInfo[2].m_fFrontGate = m_fBGateFront;
		}
		else
		{
	...
	if (m_bGateI2IsPressed)
	{
		...
		if (m_nFlawOrBottom == 0)
		{
			m_ChannelInfo[m_MapCONFIG_Channel[m_wChannelNo+5]].m_GateInfo[3].m_fFrontGate = m_fI2GateFront;
		}
		else
		{
	...
	// 	m_nDelayIn=m_fGateFrontOut;
	// 	m_nXRangeIn=m_fGateFrontOut;
	// 	m_nYRangeIn=m_fGateFrontOut;
	m_bSetOne=true;
	emit AscanInfoChanged();
	// 	m_nGainIn=m_fGateFrontOut;
	// 	m_nSuppressionIn=m_fGateFrontOut;
}
...
void AScan::GateWidthChange()	//闸门宽度变量跟踪滑块变化函数
{
	...
	if (m_bGateIIsPressed)
	{
		...
		if (m_nFlawOrBottom == 0)
		{
			m_ChannelInfo[m_MapCONFIG_Channel[m_wChannelNo+5]].m_GateInfo[0].m_fBackGate = m_fIGateWidth;
		}
		else
		{
	...
	if (m_bGateAIsPressed)
	{
		...
		if (m_nFlawOrBottom == 0)
		{
			m_ChannelInfo[m_MapCONFIG_Channel[m_wChannelNo+5]].m_GateInfo[1].m_fBackGate = m_fAGateWidth;
		}
		else
		{
	...
	if (m_bGateBIsPressed)
	{
		...
		if (m_nFlawOrBottom == 0)
		{
			m_ChannelInfo[m_MapCONFIG_Channel[m_wChannelNo+5]].m_GateInfo[2].m_fBackGate = m_fBGateWidth;
		}
		else
		{
	...
	if (m_bGateI2IsPressed)
	{
		...
		if (m_nFlawOrBottom == 0)
		{
			m_ChannelInfo[m_MapCONFIG_Channel[m_wChannelNo+5]].m_GateInfo[3].m_fBackGate = m_fI2GateWidth;
		}
		else
		{
...
void AScan::GatePeakChange()		//闸门阈值变量跟踪滑块变化函数
{
	...
	if (m_bGateIIsPressed)
	{
		...
		if (m_nFlawOrBottom == 0)
		{
			m_ChannelInfo[m_MapCONFIG_Channel[m_wChannelNo+5]].m_GateInfo[0].m_fPeakGate = m_fIGatePeak;
		}
		else
		{
	...
	if (m_bGateAIsPressed)
	{
		...
		if (m_nFlawOrBottom == 0)
		{
			m_ChannelInfo[m_MapCONFIG_Channel[m_wChannelNo+5]].m_GateInfo[1].m_fPeakGate = m_fAGatePeak;
		}
		else
		{
	...
	if (m_bGateBIsPressed)
	{
		...
		if (m_nFlawOrBottom == 0)
		{
			m_ChannelInfo[m_MapCONFIG_Channel[m_wChannelNo+5]].m_GateInfo[2].m_fPeakGate = m_fBGatePeak;
		}
		else
		{
	...
	if (m_bGateI2IsPressed)
	{
		...
		if (m_nFlawOrBottom == 0)
		{
			m_ChannelInfo[m_MapCONFIG_Channel[m_wChannelNo+5]].m_GateInfo[3].m_fPeakGate = m_fI2GatePeak;
		}
		else
		{
...
void AScan::DelayChange()		//延迟变量跟踪滑块变化函数
{
	...
	m_stAllCH_CONFIG[m_wChannelNo].stWaveConfigure[0].wDelay = MicroSecond2Dot(m_fDelay);
	m_stAllCH_CONFIG[m_wChannelNo].stWaveConfigure[1].wDelay = MicroSecond2Dot(m_fDelay);
	//m_ChannelInfo[m_wChannelNo].m_fChannelDelay = m_fDelay;
	if (m_nFlawOrBottom == 0)
	{
		m_ChannelInfo[m_MapCONFIG_Channel[m_wChannelNo+5]].m_fChannelDelay = m_fDelay;
	}
	else
	{
...
void AScan::RangeChange()		//X轴范围变量跟踪滑块变化函数
{
	...
	//m_ChannelInfo[m_wChannelNo].m_fChannelRange = m_fRange;
	if (m_nFlawOrBottom == 0)
	{
		m_ChannelInfo[m_MapCONFIG_Channel[m_wChannelNo+5]].m_fChannelRange = m_fRange;
	}
	else
	{
...
void AScan::ReferGainChange()		//参考增益变量跟踪滑块变化函数
{
	...
	if (m_nFlawOrBottom == 0)
	{
		if (m_MapCONFIG_Channel[m_wChannelNo+5] >= 25 && m_MapCONFIG_Channel[m_wChannelNo+5] <= 29)
		{
			m_ChannelInfo[m_MapCONFIG_Channel[m_wChannelNo+5] - 5].m_fChannelCPlus = m_fCGain;
		}
		m_ChannelInfo[m_MapCONFIG_Channel[m_wChannelNo+5]].m_fChannelCPlus = m_fCGain;
	}
	else
	{
		if (m_MapCONFIG_Channel[m_wChannelNo] >= 20 && m_MapCONFIG_Channel[m_wChannelNo] <= 24)
		{
			m_ChannelInfo[m_MapCONFIG_Channel[m_wChannelNo] + 5].m_fChannelCPlus = m_fCGain;
		}
...
void AScan::ReFreqChange()		//重复频率变量跟踪滑块变化函数
{
	...
	//m_ChannelInfo[m_wChannelNo].m_ProbeInfo.m_wReFreq = m_fReFreq;
	if (m_nFlawOrBottom == 0)
	{
		if (m_MapCONFIG_Channel[m_wChannelNo+5] >= 25 && m_MapCONFIG_Channel[m_wChannelNo+5] <=29)
		{
			m_ChannelInfo[m_MapCONFIG_Channel[m_wChannelNo+5] - 5].m_ProbeInfo.m_wReFreq = m_fReFreq;
		}
		m_ChannelInfo[m_MapCONFIG_Channel[m_wChannelNo+5]].m_ProbeInfo.m_wReFreq = m_fReFreq;
	}
	else
	{
		if (m_MapCONFIG_Channel[m_wChannelNo] >= 20 && m_MapCONFIG_Channel[m_wChannelNo] <= 24)
		...
		m_ChannelInfo[m_MapCONFIG_Channel[m_wChannelNo]].m_ProbeInfo.m_wReFreq = m_fReFreq;
	}
	m_bSetOne=true;
	emit AscanInfoChanged();
}
...
void AScan::GainChange()			//增益变量跟踪滑块变化函数
{
	...
	if (m_nFlawOrBottom == 0)
	{
		if (m_MapCONFIG_Channel[m_wChannelNo+5] >= 25 && m_MapCONFIG_Channel[m_wChannelNo+5] <= 29)
		{
			m_ChannelInfo[m_MapCONFIG_Channel[m_wChannelNo+5] - 5].m_fChannelPlus = m_fGain;
		}
		m_ChannelInfo[m_MapCONFIG_Channel[m_wChannelNo+5]].m_fChannelPlus = m_fGain;
	}
	else
	{
		if (m_MapCONFIG_Channel[m_wChannelNo] >= 20 && m_MapCONFIG_Channel[m_wChannelNo] <= 24)
...
void AScan::SuppressionChange()	//抑制变量跟踪滑块变化函数
{
	...
	if (m_nFlawOrBottom == 0)
	{
		m_ChannelInfo[m_MapCONFIG_Channel[m_wChannelNo+5]].m_fRestrain = m_fSuppression;
	}
	else
	{
...
{
	spinBoxGateFront->setValue((double)(horizontalSliderGateFront->value())/1000);
}
...
{
	spinBoxGateWidth->setValue((double)(horizontalSliderGateWidth->value())/1000);
}
...
{
	spinBoxGatePeak->setValue((double)(horizontalSliderGatePeak->value())/1000);
}
...
{
	spinBoxZeroPoint->setValue((double)(horizontalSliderZeroPoint->value())/1000);
}
...
{
	spinBoxDelay->setValue((double)(horizontalSliderDelay->value())/100);
}
...
{
	spinBoxSuppression->setValue((double)(verticalSliderSuppression->value())/1000);
}
...
{
	spinBoxGain->setValue((double)(verticalSliderGain->value())/1000);
}
...
void AScan::GateIPainter()
{
	...
	if (m_bGateI==1)
	{
		QPainter painter_I(this);
		painter_I.begin(this);
		painter_I.setPen(QColor(255,0,0));
		painter_I.drawLine(m_fGateILeft,m_fGateITop,m_fGateILeft,m_fGateIBottom);
		painter_I.drawLine(m_fGateILeft,m_fGateIMiddle,m_fGateIRight,m_fGateIMiddle);
		painter_I.drawLine(m_fGateIRight,m_fGateITop,m_fGateIRight,m_fGateIBottom);
		if (m_bGateIIsPressed==true)
		{
			painter_I.setBrush(QColor(0,0,255));
			QRect rectLeft(m_fGateILeft-3,m_fGateIMiddle-2,5,5);
			QRect rectRight(m_fGateIRight-3,m_fGateIMiddle-2,5,5);
			painter_I.drawRect(rectLeft);
			painter_I.drawRect(rectRight);
			painter_I.end();
			...
			char cGate1MPAmp[10];
			sprintf(cGate1SyncTime,"%d",m_CH_FLAWDATA_CSCAN_D.wGate1SyncTime);
			sprintf(cGate1MPAmp,"%d",m_CH_FLAWDATA_CSCAN_D.wGate1MPAmp);
			...
			labelGateTimeShow->setText(QObject::tr(cGate1SyncTime));
			emit AscanInfoChanged();
		}
...
void AScan::GateAPainter()
{
	...
	if (m_bGateA==1)
	{
		QPainter painter_A(this);
		painter_A.begin(this);
		painter_A.setPen(QColor(0,255,0));
		painter_A.drawLine(m_fGateALeft,m_fGateATop,m_fGateALeft,m_fGateABottom);
		painter_A.drawLine(m_fGateALeft,m_fGateAMiddle,m_fGateARight,m_fGateAMiddle);
		painter_A.drawLine(m_fGateARight,m_fGateATop,m_fGateARight,m_fGateABottom);
		if (m_bGateAIsPressed)
		{
			painter_A.setBrush(QColor(255,0,0));
			QRect rectLeft(m_fGateALeft-3,m_fGateAMiddle-2,5,5);
			QRect rectRight(m_fGateARight-3,m_fGateAMiddle-2,5,5);
			painter_A.drawRect(rectLeft);
			painter_A.drawRect(rectRight);
			painter_A.end();
			...
			float tempX = (m_CH_FLAWDATA_CSCAN_D.wXHigh16 * 65536  + m_CH_FLAWDATA_CSCAN_D.wXLow16) / (X_TO_LENGTH);


			sprintf(cXLocation,"X:%.1f--MPAmp:",tempX);
			labelGateRange->setText(QObject::tr("闸门A内最大波峰值幅度: "));
			labelGateRange->setText(QString(cXLocation));
			...
			labelGateTime->setText(QObject::tr("闸门A内最大波峰位置: "));
			...
			sprintf(cGate2MPTime,"%.1f",temp);
			labelGateTimeShow->setText(QObject::tr(cGate2MPTime));
			emit AscanInfoChanged();
		}
	}
}

void AScan::GateBPainter()
{
	if (m_bGateB==0)
	{
	...
	if (m_bGateB==1)
	{
		...
		painter_B.begin(this);
		painter_B.setPen(QColor(0,0,255));
		painter_B.drawLine(m_fGateBLeft,m_fGateBTop,m_fGateBLeft,m_fGateBBottom);
		painter_B.drawLine(m_fGateBLeft,m_fGateBMiddle,m_fGateBRight,m_fGateBMiddle);
		painter_B.drawLine(m_fGateBRight,m_fGateBTop,m_fGateBRight,m_fGateBBottom);
		if (m_bGateBIsPressed)
		{
			painter_B.setBrush(QColor(255,0,0));
			QRect rectLeft(m_fGateBLeft-3,m_fGateBMiddle-2,5,5);
			QRect rectRight(m_fGateBRight-3,m_fGateBMiddle-2,5,5);
			painter_B.drawRect(rectLeft);
			painter_B.drawRect(rectRight);
			painter_B.end();
			float fSync = (float(m_CH_FLAWDATA_CSCAN_D.wGate1SyncTime)/ ADFREQUENCY + m_fDelayOut) * m_ChannelInfo[m_wChannelNo].m_nWaveSpeed / 1000.0f / 2.0f;
			...
			char cGate3MPAmp[10];
			sprintf(cGate3MPTime,"%d",m_CH_FLAWDATA_CSCAN_D.wGate3MPTime);
			sprintf(cGate3MPAmp,"%d",m_CH_FLAWDATA_CSCAN_D.wGate3MPAmp);
			...
			labelGateTimeShow->setText(QObject::tr(cGate3MPTime));
			emit AscanInfoChanged();
		}
	}
}

void AScan::GateI2Painter()
{
	if (m_bGateI2==0)
	{
	...
	if (m_bGateI2==1)
	{
		QPainter painter_I2(this);
		painter_I2.begin(this);
		painter_I2.setPen(QColor(255,255,0));
		painter_I2.drawLine(m_fGateI2Left,m_fGateI2Top,m_fGateI2Left,m_fGateI2Bottom);
		painter_I2.drawLine(m_fGateI2Left,m_fGateI2Middle,m_fGateI2Right,m_fGateI2Middle);
		painter_I2.drawLine(m_fGateI2Right,m_fGateI2Top,m_fGateI2Right,m_fGateI2Bottom);
		if (m_bGateI2IsPressed)
		{
			painter_I2.setBrush(QColor(255,0,0));
			QRect rectLeft(m_fGateI2Left-3,m_fGateI2Middle-2,5,5);
			QRect rectRight(m_fGateI2Right-3,m_fGateI2Middle-2,5,5);
			painter_I2.drawRect(rectLeft);
			painter_I2.drawRect(rectRight);
			painter_I2.end();
			...
			char cGate4MPAmp[10];
			sprintf(cGate4SyncTime,"%d",m_CH_FLAWDATA_CSCAN_D.wGate4SyncTime);
			sprintf(cGate4MPAmp,"%d",m_CH_FLAWDATA_CSCAN_D.wGate4MPAmp);
			labelGateRange->setText(QObject::tr("闸门I2内最大波峰值幅度: "));
			labelGateTime->setText(QObject::tr("闸门I2内最大波峰值时间: "));
			labelGateRangeShow->setText(QString(cGate4MPAmp));
			labelGateTimeShow->setText(QObject::tr(cGate4SyncTime));
			emit AscanInfoChanged();
		}
	}
}
...
void AScan::InitChannelInfo()
{
	for (int i=0; i<CHANNEL_NUM; i++)
	{
		for (int j=0; j<4; j++)
		{
			...
			m_ChannelInfo[i].m_GateInfo[j].m_fFrontGate = Dot2Range(m_ChannelInfo[m_wChannelNo].m_nWaveSpeed, m_stAllCH_CONFIG[i].stWaveConfigure[m_nFlawOrBottom].stGate[j].wGateStart);
			m_ChannelInfo[i].m_GateInfo[j].m_fBackGate = Dot2Range(m_ChannelInfo[m_wChannelNo].m_nWaveSpeed, m_stAllCH_CONFIG[i].stWaveConfigure[m_nFlawOrBottom].stGate[j].wGateWidth);
	...
	for (int i = 0;i < 30;i++)
	{
		if (i >= 0 && i <= 24)
		{
			...
			for (int j = 0;j < 4;j++)
			{
				...
				m_stAllCH_CONFIG[m_MapChannel_CONFIG[i]].stWaveConfigure[m_nFlawOrBottom].stGate[j].wGateStart = Range2Dot(m_nWaveSpeed,m_ChannelInfo[i].m_GateInfo[j].m_fFrontGate) - MicroSecond2Dot(m_ChannelInfo[i].m_fChannelDelay);
				m_stAllCH_CONFIG[m_MapChannel_CONFIG[i]].stWaveConfigure[m_nFlawOrBottom].stGate[j].wGateWidth = Range2Dot(m_ChannelInfo[m_wChannelNo].m_nWaveSpeed,m_ChannelInfo[i].m_GateInfo[j].m_fBackGate);
...
void AScan::SetGateInfo(void)
{
	if (m_nFlawOrBottom == 0)
	{
		m_bGateI = m_ChannelInfo[m_MapCONFIG_Channel[m_wChannelNo+5]].m_GateInfo[0].m_bOpen ;
		m_bGateA = m_ChannelInfo[m_MapCONFIG_Channel[m_wChannelNo+5]].m_GateInfo[1].m_bOpen ;
		m_bGateB = m_ChannelInfo[m_MapCONFIG_Channel[m_wChannelNo+5]].m_GateInfo[2].m_bOpen ;
		m_bGateI2 = m_ChannelInfo[m_MapCONFIG_Channel[m_wChannelNo+5]].m_GateInfo[3].m_bOpen ;
		m_fIGateFront = m_ChannelInfo[m_MapCONFIG_Channel[m_wChannelNo+5]].m_GateInfo[0].m_fFrontGate;
		m_fAGateFront = m_ChannelInfo[m_MapCONFIG_Channel[m_wChannelNo+5]].m_GateInfo[1].m_fFrontGate;
		m_fBGateFront = m_ChannelInfo[m_MapCONFIG_Channel[m_wChannelNo+5]].m_GateInfo[2].m_fFrontGate;
		m_fI2GateFront = m_ChannelInfo[m_MapCONFIG_Channel[m_wChannelNo+5]].m_GateInfo[3].m_fFrontGate;
		m_fIGateWidth = m_ChannelInfo[m_MapCONFIG_Channel[m_wChannelNo+5]].m_GateInfo[0].m_fBackGate;
		m_fAGateWidth = m_ChannelInfo[m_MapCONFIG_Channel[m_wChannelNo+5]].m_GateInfo[1].m_fBackGate;
		m_fBGateWidth = m_ChannelInfo[m_MapCONFIG_Channel[m_wChannelNo+5]].m_GateInfo[2].m_fBackGate;
		m_fI2GateWidth = m_ChannelInfo[m_MapCONFIG_Channel[m_wChannelNo+5]].m_GateInfo[3].m_fBackGate;
		m_fIGatePeak = m_ChannelInfo[m_MapCONFIG_Channel[m_wChannelNo+5]].m_GateInfo[0].m_fPeakGate;
		m_fAGatePeak = m_ChannelInfo[m_MapCONFIG_Channel[m_wChannelNo+5]].m_GateInfo[1].m_fPeakGate;
		m_fBGatePeak = m_ChannelInfo[m_MapCONFIG_Channel[m_wChannelNo+5]].m_GateInfo[2].m_fPeakGate;
		m_fI2GatePeak = m_ChannelInfo[m_MapCONFIG_Channel[m_wChannelNo+5]].m_GateInfo[3].m_fPeakGate;
	}
	else
	{
...
void AScan::SetProbeInfo(void)
{
	if (m_nFlawOrBottom == 0)
	{
		m_fDelay = m_ChannelInfo[m_MapCONFIG_Channel[m_wChannelNo+5]].m_fChannelDelay;
		int nMin = Dot2Range(m_ChannelInfo[m_wChannelNo].m_nWaveSpeed, MONITOR_SIZE);
		horizontalSliderXRange->setRange(nMin,A_SCAN_MAX_RANGE);
		if (m_ChannelInfo[m_MapCONFIG_Channel[m_wChannelNo+5]].m_fChannelRange < nMin)
		{
			m_ChannelInfo[m_MapCONFIG_Channel[m_wChannelNo+5]].m_fChannelRange = nMin;
		}
		m_fRange = m_ChannelInfo[m_MapCONFIG_Channel[m_wChannelNo+5]].m_fChannelRange;
		m_fCGain = m_ChannelInfo[m_MapCONFIG_Channel[m_wChannelNo+5]].m_fChannelCPlus;
		m_fGain = m_ChannelInfo[m_MapCONFIG_Channel[m_wChannelNo+5]].m_fChannelPlus;
		//更新抑制
		m_fSuppression = m_ChannelInfo[m_MapCONFIG_Channel[m_wChannelNo+5]].m_fRestrain;
		//更新零点
		m_fZeroPoint = m_ChannelInfo[m_MapCONFIG_Channel[m_wChannelNo+5]].m_fZeroPoint;
		//更新重复频率
		m_fReFreq = m_ChannelInfo[m_MapCONFIG_Channel[m_wChannelNo+5]].m_ProbeInfo.m_wReFreq;
		DWORD dwDelay = MicroSecond2Dot(m_fDelay);
		m_fXMinRuler = Dot2Range(m_ChannelInfo[m_wChannelNo].m_nWaveSpeed, dwDelay);
		m_fXMaxRuler = m_fRange + m_fXMinRuler;
		emit AscanInfoChanged();
	...
	{
		m_fDelay = m_ChannelInfo[m_MapCONFIG_Channel[m_wChannelNo]].m_fChannelDelay;
		int nMin = Dot2Range(m_ChannelInfo[m_wChannelNo].m_nWaveSpeed, MONITOR_SIZE);
		horizontalSliderXRange->setRange(nMin,A_SCAN_MAX_RANGE);
		if (m_ChannelInfo[m_MapCONFIG_Channel[m_wChannelNo]].m_fChannelRange < nMin)
		{
			m_ChannelInfo[m_MapCONFIG_Channel[m_wChannelNo]].m_fChannelRange = nMin;
		}
		m_fRange = m_ChannelInfo[m_MapCONFIG_Channel[m_wChannelNo]].m_fChannelRange;
		m_fCGain = m_ChannelInfo[m_MapCONFIG_Channel[m_wChannelNo]].m_fChannelCPlus;
		m_fGain = m_ChannelInfo[m_MapCONFIG_Channel[m_wChannelNo]].m_fChannelPlus;
		//更新抑制
		m_fSuppression = m_ChannelInfo[m_MapCONFIG_Channel[m_wChannelNo]].m_fRestrain;
		//更新零点
		m_fZeroPoint = m_ChannelInfo[m_MapCONFIG_Channel[m_wChannelNo]].m_fZeroPoint;
		//更新重复频率
		m_fReFreq = m_ChannelInfo[m_MapCONFIG_Channel[m_wChannelNo]].m_ProbeInfo.m_wReFreq;
		DWORD dwDelay = MicroSecond2Dot(m_fDelay);
		m_fXMinRuler = Dot2Range(m_ChannelInfo[m_wChannelNo].m_nWaveSpeed, dwDelay);
		m_fXMaxRuler = m_fRange + m_fXMinRuler;
		emit AscanInfoChanged();
...
void AScan::ChangeMapCPicData(WORD wChannelNum)
{
	...
	if (!bExist)
	{
		CPicData* pCPicData = NULL;
		pCPicData = new CPicData;
		m_MapCPicData[wChannelNum] = pCPicData;
	}
...
void AScan::timerEvent( QTimerEvent *event )
{
	...
	else
	{
		...
		if(event->timerId()==m_nTIMER_Sel_Channel)
		{
			...
			{
				i++;
				if (i == m_wMapCurrentChannelNum)
				{
					if (itPicData->first > 29)
					{
//						g_dwActiveChannel = itPicData->first - 5;
//						g_dwActiveFlawOrBottom = 0;
						m_bSelChannel = TRUE;
					}
					else
					{
//						g_dwActiveChannel = itPicData->first;
...
{
	int i = 0;
	int j = comboBoxProbe->count();
	for ( i ; i < j ; i ++ )
	{
		comboBoxProbe->removeItem(0);
	}
