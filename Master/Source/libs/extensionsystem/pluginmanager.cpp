...
{
    PluginManager::splashMessage(tr("Resolving plugin dependencies..."));
    QList<PluginSpec *> queue = loadQueue();
//    Utils::setMimeStartupPhase(MimeStartupPhase::PluginsLoading);
    foreach (PluginSpec *spec, queue) {
        PluginManager::splashMessage(tr("Loading plugin %1...").arg(spec->name()));
        loadPlugin(spec, PluginSpec::Loaded);
    }
//    Utils::setMimeStartupPhase(MimeStartupPhase::PluginsInitializing);
    foreach (PluginSpec *spec, queue) {
        PluginManager::splashMessage(tr("Initializing plugin %1...").arg(spec->name()));
        loadPlugin(spec, PluginSpec::Initialized);
    }
//    Utils::setMimeStartupPhase(MimeStartupPhase::PluginsDelayedInitializing);
    Utils::reverseForeach(queue, [this](PluginSpec *spec) {
        PluginManager::splashMessage(tr("Starting plugin %1...").arg(spec->name()));
...
void PluginManager::setSplash(QSplashScreen *splash)
{
    d->splash = splash;
}

void PluginManager::splashMessage(const QString &message, const QColor &color)
{
    if (d->splash) {
        d->splash->showMessage(message, Qt::AlignHCenter | Qt::AlignVCenter, color);
    }
}
