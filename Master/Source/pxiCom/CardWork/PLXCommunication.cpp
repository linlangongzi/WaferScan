...
CPLXCommunication::E_InitResult CPLXCommunication::Init(SharedCardParamVec pstThreadParam, int nInsertAecNum)
{
    DWORD dwRet = PLX_LibInit();
    if (dwRet != WD_STATUS_SUCCESS)
    {
        QMessageBox::warning(NULL, "warning", QObject::tr("PLX初始化失败"), QMessageBox::Yes);
        m_eInitResult = ERROR_INIT;
        return ERROR_INIT;
    }
    ...
    if (!bRet)
    {
        ...
        qWarning() << "PLX Bus、Slot查询失败!";
        m_eInitResult = ERROR_FIND;
        return m_eInitResult;
    }

    //if(m_wTotalCardNum <= nInsertAecNum)
    if(m_wAECCardNum < nInsertAecNum)
    {
        qWarning() << "没有查找出所有AEC卡";
        m_eInitResult = ERROR_AEC_CARD_NUM;
        return m_eInitResult;
    }
    ...
    if (!bRet)
    {
        ...
        {
            PLX_DeviceClose(pstThreadParam->at(i)->hDev);
        }

        //		WriteLog(_T("NDT-EventLog.txt"), _T("卡自检失败!"),CFile::modeReadWrite|CFile::modeCreate);
        qWarning() << "卡自检失败!";
        m_eInitResult = ERROR_SELTEST;
        return ERROR_SELTEST;
    }
    else
    {
        //		WriteLog(_T("NDT-EventLog.txt"), _T("卡自检成功!"),CFile::modeReadWrite|CFile::modeCreate);
        qWarning() << "卡自检成功";
        m_eInitResult = SUCCESS;
        return SUCCESS;
    }
}

DWORD CPLXCommunication::UnInit(SharedCardParamVec pstThreadParam)
{
    for(int i=0; i< m_wTotalCardNum; i++)
    {
        P9054_Close(pstThreadParam->at(i)->hDev);
    }

    return PLX_LibUninit();
}

BOOL CPLXCommunication::AllCardsSelfTest(SharedCardParamVec pstThreadParam)
{
    BOOL bselftest = FALSE;

    for (int i = 1;i <= m_wAECCardNum;i++)  //aec卡自检，从thread_param[1]开始 <dwCardCount ----><=g_nAECCardNum
    {
        bselftest = CardSelfTest(pstThreadParam->at(i)->hDev);
...
//    bselftest = MultiCardTest(pstThreadParam->at(0)->hDev);   //多功能卡自检，应该用g_nMFCNum;
//    bselftest = true;   //多功能卡自检，应该用g_nMFCNum;
...
//    else
//    {
//        WORD wMPBVer = P9054_ReadWord(pstThreadParam->at(0)->hDev, P9054_ADDR_SPACE0, 0x00);
//        /*
//        CString strMPBVer;
...
BOOL CPLXCommunication::CardSelfTest(P9054_HANDLE hDev)
{
    ...
    while (dwAddr < SYS_SELF_TEST_ADDRESS_END) {
        ...
        if (wValueRead != wValueWrite) {
            ...
            if (nResetCount == 3) {
                QMessageBox::information(NULL, "Error", QString(QObject::tr("AEC卡自检在地址:%1处出现读写错误")).arg(errorAddress), QMessageBox::Yes);
                return FALSE;
            }
...
*/
BOOL CPLXCommunication::BusSlotFindAndSort(SharedCardParamVec pstThreadParam)
{
    // 计算板卡数量
    ...
    else
    {
        ...
        for(int i = 0; i < m_wTotalCardNum; i++)  //查找卡
        {
            ...
            else
            {
                ...
                case MULTI_CARD_FLAG:
                    pstThreadParam->at(0)->hDev = hPlx;
                    m_BusSlotIndex[0] = (*hPlx).pciSlot;
                    break;
                case AEC_CARD_FLAG:
                    pstThreadParam->at(j)->hDev = hPlx;
        ...
        for(int k=1; k<m_wTotalCardNum; k++)
        {
            ...
            {
                if(l != k &&
                    (m_BusSlotIndex[k].dwBus == pstThreadParam->at(l)->hDev->pciSlot.dwBus) &&
                    (m_BusSlotIndex[k].dwFunction == pstThreadParam->at(l)->hDev->pciSlot.dwFunction) &&
                    (m_BusSlotIndex[k].dwSlot == pstThreadParam->at(l)->hDev->pciSlot.dwSlot))
                {
                    hPlx = pstThreadParam->at(k)->hDev;
                    pstThreadParam->at(k)->hDev = pstThreadParam->at(l)->hDev;
                    pstThreadParam->at(l)->hDev = hPlx;
                }
            }
        }
...
//
////////////////////////////////////////////////////////////////
void CPLXCommunication::SetMonitorRange(P9054_HANDLE hDev, quint16 *pInterSpace, quint32 unAddr)
{
    //DWORD dwAddr = MONITOR_RANGE_ADDRESS;
    /*for (int i = 0;i < FLAW_DOT_MAX_NO;i++)
    {
        P9054_WriteWord(hDev, P9054_ADDR_SPACE0, unAddr, pInterSpace[i]);
        unAddr += 2;
    }*/
    // DMA写
    int nSize = FLAW_DOT_MAX_NO * 2;
    P9054_DMAReadWriteBlock(hDev, unAddr, pInterSpace,
                nSize, FALSE, P9054_MODE_WORD, P9054_DMA_CHANNEL_0);
}

/*!
 * \brief CPLXCommunication::ReadMonitorData 读取画图数据
 * \param hDev
 * \param pData
 * \param unEnAddr 就绪标志位
 * \param unAddr 缓冲区地址
 * \param bufferIndex 缓冲区序号 0~7
 * \return 读取成功标志
 */
bool CPLXCommunication::ReadMonitorData(P9054_HANDLE hDev, unsigned short *pData,
                                        quint32 unEnAddr, quint32 unAddr, quint32 bufferIndex)
{
    if ( ( (0x1 << bufferIndex) & P9054_ReadWord(hDev, P9054_ADDR_SPACE0, unEnAddr) ) == 0 ) {  // 验证传入序号的缓冲区是否就绪
        return false;
    }

#ifdef PIPE_DETECT
//    for (int i = 0; i < 512; ++i) {
//        pData[i] = P9054_ReadWord(hDev, P9054_ADDR_SPACE0, unAddr);
//        unAddr += 0x2;
//    }

    P9054_DMAReadWriteBlock(hDev, unAddr, pData, MONITOR_SIZE_DMA, TRUE, P9054_MODE_WORD, P9054_DMA_CHANNEL_0);
#else
    for (int i = 0; i < 512; ++i) {
        pData[i] = P9054_ReadWord(hDev, P9054_ADDR_SPACE0, unAddr);
        unAddr += 0x2;
...
CPLXCommunication::E_InitResult CPLXCommunication::GetInitResult()
{
    return m_eInitResult;
}
