...
QVector3D BSplineCurve::PointOnCurve(qreal u)
{
    if(ctrlPoints.cols == 3) {
        return QVector3D(PointOnCurve(u, Qt::XAxis),
                         PointOnCurve(u, Qt::YAxis),
                         PointOnCurve(u, Qt::ZAxis));
    } else {
        return QVector3D();
    }
}
...
BSplineCurve::Matrix BSplineCurve::CurveSubdivide(const Matrix &points, const int numOfSubdivide)
{
    ...
    while (u < 1) {
        ...
        lengthOfCurve +=  du * qSqrt(qPow(tempPoint(0, 1), 2) + qPow(tempPoint(1, 1), 2) + qPow(tempPoint(2, 1), 2));
    }
