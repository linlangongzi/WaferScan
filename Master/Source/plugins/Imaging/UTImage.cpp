...
{
    return uint(data()->data.cols) > uint(x) && uint(data()->data.rows) > uint(y);
}
 ...
 * \sa SetPointFast
 */
void UTImage::SetPoint(UTDataFrame frame, const ImageOptions &trans, int index, int xIndex, int yIndex)
{
 ...
 * \sa SetPointsFast
 */
void UTImage::SetPoints(const UTDataSegment &segment, const ImageOptions &trans, int index, int xIndex, int yIndex)
{
    qDebug() << "UTImage::SetPoints writing" << segment.count() << "frames";
    int count = 0;
    for (const auto &frame : segment) {
        SetPoint(frame, trans, index, xIndex, yIndex);
        ++count;
    }
 ...
 * \sa SetPoint
 */
void UTImage::SetPointFast(UTDataFrame frame, const ImageOptions &trans, int index, int xIndex, int yIndex)
{
 ...
 * \sa SetPoints
 */
void UTImage::SetPointsFast(const UTDataSegment &segment, const ImageOptions &trans, int index, int xIndex, int yIndex)
{
    for (const auto &frame : segment) {
        SetPointFast(frame, trans, index, xIndex, yIndex);
    }
}
#endif
...
{
    UTImagePrivate::Image32 rendered = data()->Render(palette, mask ? mask->data() : nullptr);
...
{
    UTImagePrivate::Image8 bytes = data()->Render8(min, max, mask ? mask->data() : nullptr);
...
void UTImage::GetMinMax(CDataValue *min, CDataValue *max, QPoint *minLoc, QPoint *maxLoc, const UTImage *mask, const QRect &ROI) const
{
    ...
    Point maxLocBuffer;
    Mat image;
    Mat maskBytes = data()->PrepareMask(mask ? mask->data() : nullptr, ROI, &image, false);
    cv::minMaxLoc(image,
                  min ? &minBuffer : nullptr, max ? &maxBuffer : nullptr,
                  minLoc ? &minLocBuffer : nullptr, maxLoc ? &maxLocBuffer : nullptr,
...
void UTImage::meanStdDev(CDataValue *mean, CDataValue *stddev, const UTImage *mask, const QRect &ROI) const
{
    if (!IsValid()) {
        return;
    }
    Scalar m, s;
    cv::meanStdDev(data()->SubImage(ROI), m, s, mask ? mask->data()->ToMask() : noArray());
    if (mean) {
        *mean = m[0];
    }
    if (stddev) {
...
UTImage::Boundaries UTImage::FindBoundaries(CDataValue threshold, UTImage::ThresholdMode mode, const QRect &ROI, const UTImage *mask) const
{
    ...
    QRect roi = BoundRect(ROI);
    Mat1b bytes = data()->SubImage8(mode, -threshold * mode, roi, mask ? mask->data() : nullptr);
...
{
    Mat subImage;
    Mat maskBytes = data()->PrepareMask(mask ? mask->data() : nullptr, ROI, &subImage, false);
...
UTDefectList UTImage::FindDefects(const UTImageList &images, const UTDefectProcessorOptions &options) const
{
    QElapsedTimer timer;
    timer.start();

    const UTImage surface = images.value(0);

    const UTImage depth = images.value(options.depthIndex);
    const UTImage bottomDepth = images.value(options.bottomDepthIndex);
    /*! 缺陷图像
     * 0: 无缺陷
     * 2: 有缺陷，未分类
     * 2~(n+2): 缺陷编号 */
    Mat threshold;
    UTImage::CompareMode mode = (options.thresholdMode == UTDefectProcessorOptions::LOW) ? UTImage::CMP_GE : UTImage::CMP_LE;
    cv::compare(data()->data, options.threshold, threshold, mode);
//    imshow("Threshold", threshold);

    Mat1i image;
    threshold.convertTo(image, CV_32S, 1.0f / 255);                             /* cv::compare为true的部位，填充值为255 */
//    qDebug() << "Find defects: got " << cv::countNonZero(image) << "pixels";

    if (!options.roi.isEmpty()) {
//        qDebug() << "Applying ROI at" << options.roi;
        Mat1b mask(data()->data.size(), 1);
        const auto *points = reinterpret_cast<const Point *>(options.roi.data());
        const int numPoints = options.roi.count();
        cv::fillPoly(mask, &points, &numPoints, 1, Scalar(0));                  /* 多边形内填充为0 */
//        qDebug() << "Mask has " << cv::countNonZero(mask) << "pixels";
        image.setTo(0, mask);                                                   /* 将缺陷图像中非0部位清零 */
    }
//    qDebug() << "Find defects: got " << cv::countNonZero(image) << "pixels after roi filter";

    UTDefectList defects;
    Rect rect;
    for (int row = 0; row < image.rows; ++row) {                                /* cv::Mat为行主序，先遍历行效率更高 */
        for (int col = 0; col < image.cols; ++col) {
            if (!qFuzzyIsNull(surface(col, row)) && image(row, col) == 1) {
//                qDebug() << "Filling at" << row << col;
                /* 将缺陷位置填充为"index + 2" */
                cv::floodFill(image, Point(col, row), Scalar(defects.count() + 2), &rect);

                UTDefect d;
                d.markRect = QRect(rect.x, rect.y, rect.width, rect.height);
                d.image = QImage(QSize(rect.width, rect.height), QImage::Format_Mono);
                d.image.fill(Qt::color1);
                defects << d;
            }
        }
    }

    /* 遍历所有像素，按照index填充defect列表中的像素 */
    for (int row = 0; row < image.rows; ++row) {
        for (int col = 0; col < image.cols; ++col) {
            int defectIndex = image(row, col) - 2;
            if (defectIndex < 0 || defectIndex > defects.count()) {
                continue;
            }

            UTDefect &defect = defects[defectIndex];
            defect.image.setPixel(col - defect.markRect.left(), row - defect.markRect.top(), Qt::color0);
            defect.pixelCount++;

            if (options.hasDepth) {
                auto d = depth(col, row);
                defect.averageDepth += d;
                defect.minDepth = qMin(defect.minDepth, d);
                defect.maxDepth = qMax(defect.maxDepth, d);
            }
            if (options.hasBottomDepth) {
                auto d = bottomDepth(col, row);
                defect.bottomDepth += d;
            }
            defect.maxAmp = qMax(defect.maxAmp, (*this)(col, row));
            defect.minAmp = qMin(defect.minAmp, (*this)(col, row));
        }
    }

    qreal speedFactor = options.materialVelocity / 2000;
    for (auto &defect: defects) {
        defect.maxDepth *= speedFactor;
        defect.minDepth *= speedFactor;
        defect.averageDepth *= speedFactor / defect.pixelCount;
        defect.bottomDepth *= speedFactor / defect.pixelCount;
    }

    qDebug() << "FindDefects took time" << timer.elapsed();
    return defects;
}
...
{
    if (IsValid()) {
        const auto *points = reinterpret_cast<const Point *>( polygon.data() );
...
UTImage UTImage::FromAData(const UTDataSegment &segment, Qt::Orientation direction, int offset, bool relativeOffset)
{
    if (segment.isEmpty()) {
        return UTImage();
    }
    ...
    const int imageSize = segment.size();
    const bool isVertical = (direction == Qt::Vertical);
    UTImage image = isVertical ? UTImage(waveSize, imageSize) : UTImage(imageSize, waveSize);
    ...
    for (int index = 0; index < imageSize; ++index) {
        ...
        if (relativeOffset && size > 0) {
            qreal syncOffset = frame.syncTime();
            qreal range = frame.Context.GetRange();
            offsetIndex -= syncOffset / range * size;
        }
        const int waveformStart = qMax(0, -offsetIndex);
        const int waveformEnd = qMin(waveform.count(), waveform.count() - offsetIndex);
        for (int waveIndex = waveformStart; waveIndex < waveformEnd; ++waveIndex) {
            if (isVertical) {
                image(waveIndex + offsetIndex, index) = waveform[waveIndex];
            } else {
                image(index, waveIndex + offsetIndex) = waveform[waveIndex];
            }
        }
    }
    return image;
...
QPolygonF FindEnclosingRect(const QPolygonF &polygon)
{
    ...
    for (const auto &rectPoint : rectPoints) {
        result << QPointF(rectPoint.x, rectPoint.y);
    }
    return result;
}
...
using Edge = const QRect *;
using EdgeMap = QMultiMap<int, Edge>;                                           /*!< 排序的边缘描述表，使用迭代器遍历此描述表即可按顺序遍历所有边缘 */
using EdgeSet = QSet<Edge>;
...
QPair<int, QRect> FindMaxObjectDensity(const QVector<QRect> &objects, const QSize &block, const QRect &ROI)
{
    ...
    using Iterator = EdgeMap::ConstIterator;
